...
> 📅 **2026-09-22 cloud_live pull 보강** (3 커밋)
>
> 세 커밋 모두 `BE3-969` 이고, **배포 워크플로 정리**가 주제입니다.
>
> | 변경 | 내용 |
> |---|---|
> | 워크플로 통합 | dev·rc1·rc4 를 **단일 파일**로 합쳤습니다. 온프레미스 잔재(`odev`·`rc`·`rc2`·`rc3`) 워크플로 4개를 지웠습니다 |
> | 운영 배포 정리 | **죽어 있던 `live`(master) 워크플로를 제거**하고, `clive` 를 `live` 로 개명했습니다 |
> | 설치 스크립트 | `setup-neo4j.sh` 가 **계정을 보고 S3 버킷을 스스로 고르도록** 바꿨습니다 |
>
> `clive` 를 `live` 로 부르기 시작한 점이 눈에 띕니다.
> 클라우드가 유일한 운영 환경이 되면서 구분할 이유가 없어진 것으로 보입니다. 이 판단은 추측입니다.
>
> 인프라 쪽 짝이 있습니다.
> [`helm-charts-gitops`](./helm-charts-gitops.md) 에서 **rc1 의 Kafka 소비를 껐습니다.** 변경 데이터 감지(CDC) 연결 전까지 막아 둔 것입니다.
