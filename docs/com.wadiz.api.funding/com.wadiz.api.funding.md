...
>
> 결제 금액을 클라이언트가 보내는 값에 기대던 자리를 **서버 계산으로 바꿨습니다.**
>
> | 고친 것 | 내용 |
> |---|---|
> | 금액 산출 일원화 | 주문서 결제 금액 계산을 `CalculateBillingUsecase` 한 곳으로 모았습니다 |
> | 수량 검증 (취약점 2) | 리워드 수량의 **부호와 상한**을 검사하고 금액 오버플로를 막습니다 |
> | 할인액 대조 (취약점 3) | 해외 결제의 **쿠폰·포인트 할인액을 서버에서 다시 확인**합니다 |
> | 음수 불변식 | 리워드 금액이 음수가 되지 못하게 했습니다. **배송비가 음수 합계를 가리던 경로**를 막았습니다 |
>
> 커밋 제목에 "취약점 2", "취약점 3" 이라고 번호가 붙어 있습니다.
> 점검에서 나온 항목을 순서대로 막는 작업으로 보입니다. 이 판단은 추측입니다.
>
> ⚠️ 중간에 `ServiceType` 의 `isKorea` 판정을 바꿨다가 **로케일 기준으로 원복**했습니다.
>
> [`com.wadiz.web`](../com.wadiz.web.md) 에서도 같은 이슈로 **청약 완료 API 진입점을 차단**했습니다.
>
> 검증 테스트가 크게 붙었습니다.
> `CreateOrderSheetExchangeRateTest` +200줄, `CreateOrderSessionUsecaseTest` 115줄, `CreateOrderSheetUsecaseTest` 98줄입니다.
>
> ### RWD-5954 — PD 컨설팅 신청형(V3)
>
> - **PD 컨설팅 V3(신청형) 버전을 신설**하고 기본값으로 뒀습니다. 제출 후 저장을 막는 가드를 넣고 쓰이지 않던 승인 API 를 지웠습니다.
> - 로컬 요금제 적용 조건에 **대표 카테고리 제한**을 더했습니다(`PricingPolicyVersion` V3 신설).
> - 어드민 화면은 [`co.wadiz.adm`](../co.wadiz.adm.md), 메이커 화면은 [`wadiz-frontend`](../wadiz-frontend/wadiz-frontend.md) FE2-1297 입니다.

> 📅 **2026-09-17 master pull 보강** (14 커밋)
>
> **인공지능 심사 알림 정리(RWD-5975, 12커밋)** 가 대부분입니다.
>
> ### RWD-5975 — 인공지능 심사 비노출 알림을 따로 뺐습니다
>
> 프로젝트가 인공지능 심사에서 비노출 판정을 받으면 메이커에게 알립니다.
> 종전에는 이 알림이 관리자 알림과 한 덩어리였습니다. 이번에 갈랐습니다.
>
> | 변경 | 내용 |
> |---|---|
> | 알림 분리 | 인공지능 심사 비노출 알림을 **관리자 알림과 분리**했습니다. 메일 템플릿 번호는 1542 이고 언어별 링크를 씁니다 |
> | 알림톡 템플릿 신설 | 비노출 알림톡을 **신규 템플릿 3231** 로 분리했습니다. 작성 언어를 넣는 치환자를 추가했습니다 |
> | 앱링크 경로 통일 | 비노출·심사 결과 알림톡의 앱링크를 **스킴(`wadiz://` 같은 접두)이 없는 경로**로 통일했습니다 |
> | 슬랙 링크 | 슬랙 알림의 스튜디오 링크에 **심사 언어**를 적용했습니다 |
>
> **심사 대상과 이력도 손봤습니다**
>
> - 오픈예정이 적용되지 않은 프로젝트는 **인공지능 심사 요청에서 오픈예정 스토리를 제외**합니다.
> - 인공지능 심사 결과를 **사후심사 이력에 기록**합니다. 신규 모델 `AIReviewRevivalHistory`(28줄)가 들어왔습니다.
> - 이력에 `StoryVersion` 을 남기던 것을 없앴습니다.
> - 쓰이지 않던 `sendAIAlarmType2` 와 `getCampaignDraftedStoryById` 쿼리를 지웠습니다.
>
> ### RWD-6026 — 친구톡 이미지 업로드를 v3 로 바꿨습니다
>
> 친구톡은 카카오톡으로 보내는 마케팅 메시지입니다.
>
> - 이미지 업로드를 **v3 방식으로 전환**하고, **카탈로그 규격 마커**를 저장하도록 했습니다.
> - 신규 `ImageVariantType`(14줄)과 `ImageUploadResult` 확장이 들어왔습니다.
> - 같은 기간 [`kr.wadiz.platform.api.friendtalk`](./../display-platform-services-3-messaging.md) 에서 **비율별 이미지 업로드 API**(DISPLAY-1726)가 만들어졌습니다. 두 작업이 짝입니다.
>
> ### 그 밖에
>
> - 로컬 프로파일에서 **Stripe SQS 리스너를 끄는 조건**을 넣었습니다. 로컬 개발 중 결제 대기열을 듣지 않게 하려는 조치입니다.
> - 로컬 프로파일의 `mail-normal` 게이트웨이 경로를 고쳤습니다.
> - `funding-core` 의존성을 **1.0.150-SNAPSHOT** 으로 올렸습니다. [`co.wadiz.adm`](../co.wadiz.adm.md)·[`com.wadiz.web`](../com.wadiz.web.md) 도 같은 버전으로 함께 올렸습니다.

> 📅 **2026-09-15 master pull 보강** (26 커밋)
>
> **새소식 예약 발행(RWD-6036·6037·6038, 20커밋)** 한 덩어리가 대부분입니다. 메이커가 새소식을 미리 써 두고 **원하는 시각에 자동 공개**되게 하는 기능입니다.
>
> ### RWD-6036 — 예약 저장·수정 API
> - 새소식에 **상태(status) 계약**을 도입하고 예약 검증·저장 이력을 확장했습니다. 신규 `NewsReservationValidator`(테스트 100줄).
> - **지난 시각으로는 예약을 접수하지 않습니다** — `RESERVATION_TIME_PASSED`. 단 **같은 값을 그대로 다시 보내는 경우(에코)는 예외**로 허용합니다. 수정 화면이 기존 값을 그대로 되돌려 보낼 때 막히지 않게 하려는 배려입니다.
> - **예약 시각이 도래한 건은 이미 발행된 것으로 취급**해 시각 변경·취소를 거부합니다. 발행 확정 배치와 동시에 손대는 경합을 막기 위해서입니다.
> - **번역 처리 시점이 두 번 바뀌었습니다** — 예약 저장 시 **즉시 번역**합니다(예약 시각이 도래해 노출되는 구간에 서포터가 보는 제목·본문이 비어 있던 문제). 조회 시점의 지연 번역 대상에도 예약 건을 포함하되 임시저장만 제외합니다.
> - 예약 건의 `newsType` 은 **DB 확정값만** 내려줍니다(발행 전이면 `null`). 예상값을 파생해 내려주던 것을 제거했습니다.
>
> ### RWD-6038 — 발행 가능 기한(`availableUntil`)
> - 응답에 **언제까지 발행할 수 있는지**를 항상 내려줍니다. 경계 정의를 여러 번 다듬었습니다.
>   - `availableUntil` 을 **마지막 포함 시각**으로 정정(발행 허용 창과 경계 일치)
>   - 발행 한도를 **실제 종료 시각 기준, 그 날 끝(23:59:59)** 으로 통일
>   - 환불 종료일이 아직 정해지지 않은 성공 캠페인은 **현재 + 6개월** 을 잠정 상한으로
>   - 기준일 미확정이면 보수적/잠정 상한으로 대체해 **값이 비는 일이 없게** 함
> - **와디즈 공지 권한은 예약 기간 검증에서 제외**합니다(`availability`·발행 확정과 같은 기준).
> - 미발행 건의 `whenPosted` 노출을 제한하고, 발행 완료 건도 `whenReserved` 를 노출합니다(예약 발행 이력의 의미).
>
> ### RWD-6037 — 예약 발행 확정
> - 알림 발송 배치의 **선행 단계**로 예약 발행을 확정하는 경로를 만들었습니다.
> - **배치 감사 사용자를 등록**해 `UpdateUserId NOT NULL` 위반을 해결했습니다 — 사람이 아니라 배치가 발행할 때 주체가 비어 있어 저장이 실패하던 문제입니다.
>
> ### RWD-6041 — 새소식 번역 저장을 upsert 로 단일화
> - 번역 저장의 **잠금 대기를 줄였습니다** — 번역을 모두 끝낸 뒤 일괄 저장하고, 저장을 단일 upsert 로 합치며, **트랜잭션 범위를 저장 구간으로만 한정**했습니다.
> - 그 과정에서 **갱신 주체 기록이 `null` 로 덮이는 회귀**가 생겨 복원했습니다.
>
> ### RWD-6027 — 취소·반환 원장에 행위 주체 기록
> - 결제 취소·환불 원장에 **누가 했는지**(`CancelActor`)를 남깁니다. 주체가 없는 커맨드도 취소 자체를 막지는 않습니다.
> - [`co.wadiz.adm`](../co.wadiz.adm.md)·[`com.wadiz.web`](../com.wadiz.web.md) 에도 같은 이슈로 `RefundActorType` 이 들어갔고, 웹은 **메이커 승인 경로의 반환 주체를 `MAKER` 로** 기록합니다.
>
> ### 🔗 프론트 짝
> 스튜디오 화면은 [`wadiz-frontend`](../wadiz-frontend/wadiz-frontend.md) 의 **FE2-1255**(20커밋 — 예약 공개 UI·안내 모달·기한 제한·잠금)입니다.
