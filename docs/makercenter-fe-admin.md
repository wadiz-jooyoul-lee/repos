...
>
> 어드민 화면에서 사용자 서비스로 넘어가는 주소를 **와디즈 통합 도메인(`ko` 접두 경로)** 으로 바꿨습니다.
> `.env`·`.env.clive`·`CLAUDE.md` 세 파일의 한 줄씩입니다.
>
> 메이커센터 주소 이전(FE2-1231)과 같은 흐름입니다. 자세한 내용은 [`makercenter-fe`](./makercenter-fe.md) 를 참고하세요.
