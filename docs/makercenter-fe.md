...
> 📅 **2026-09-22 cloud_live pull 보강** (5 커밋)
>
> ### FE2-1331 — 서버 렌더링이 원 사용자의 브라우저 정보를 넘깁니다
>
> 서버에서 화면을 미리 그릴 때(SSR) 백엔드를 부릅니다.
> 종전에는 그 호출에 **서버 자신의 정보**가 담겼습니다. 이제 **원 클라이언트의 `user-agent` 를 그대로 전달**합니다.
>
> 한글이 든 `user-agent` 가 깨지지 않는지 검증하는 테스트를 실제 런타임 입력 형태로 고쳤습니다
> (`src/shared/api/serverFetch.test.ts`, 65줄 신설).
>
> ### FE2-1379 — Datadog 성능 추적 붙임
>
> `dd-trace` 를 먼저 읽도록 해서 요청 성능을 추적합니다.
>
> ### 도메인 정리
>
> | 이슈 | 내용 |
> |---|---|
> | FE2-1331 | dev 정적 자원 CDN 호스트를 `cdn-static` 으로 교체 |
> | FE2-1355 | 남은 `wadiz.kr` 도메인을 `wadiz.io` 매핑으로 치환 |
