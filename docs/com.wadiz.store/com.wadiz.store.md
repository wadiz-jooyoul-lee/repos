...
- **엔드포인트 전수**: [`api-endpoints.md`](./api-endpoints.md) — 73개 컨트롤러 × **254 엔드포인트**
...
>
> 주문서는 결제 전 단계의 임시 데이터입니다. 일정 시간이 지나면 Redis 에서 자동으로 사라집니다.
> 그 만료 이벤트를 받아 처리하는데, **결과가 남지 않아 무슨 일이 일어났는지 볼 수 없었습니다.**
>
> - `DeleteOrderSheetService` 에 처리 결과 로그를 넣었습니다.
> - `RedisKeyExpiredEventListener` 에서 **만료 이벤트를 흘려보낼 때(drop) 남기는 로그를 자기 keyspace 로 한정**했습니다.
>   keyspace 는 Redis 에서 키를 담는 논리적 공간입니다.
>   한정하지 않으면 다른 서비스의 만료 이벤트까지 로그에 섞입니다.
...
| 컬렉션/큐레이션/프로모션/연동 | 13 | 36 | 컬렉션·큐레이션, 할인 프로모션, 첨부, SCM, 외부 카탈로그 피드, Linkprice CPS |
| **합계** | **73** | **254** | |
