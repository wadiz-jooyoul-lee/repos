...
> 📅 **2026-09-22 cloud_live pull 보강** (6 커밋)
>
> ### FE2-1390 — 홈 구좌 하나가 전체를 무너뜨리지 않게
>
> - ⚠️ **구좌 하나를 못 가져오면 홈 응답 전체가 실패하던 문제**를 고쳤습니다.
> - 홈 '와디즈 알아보기' 첫 구좌를 **펀딩 성공 가이드 게시판**으로 바꿨습니다.
> - 카드형으로 바뀐 자주 묻는 질문 구좌에 `platform` 값을 넘기도록 고쳤습니다.
>
> 검증 테스트 `MainControllerTest`(160줄)가 새로 붙었습니다.
>
> ### FE2-1355 — `wadiz.kr` 잔재 제거
>
> - 남아 있던 `wadiz.kr` 도메인을 `wadiz.io` 매핑으로 바꿨습니다.
> - **교차 출처 허용 목록에서 `wadiz.co` 를 빼고** 레거시 메이커센터 주소를 교체했습니다.
> - 쓰지 않는 프로필 설정 파일과 레거시 EC2 배포 워크플로를 지웠습니다(−537줄).

> 📅 **2026-09-17 cloud_live pull 보강** (5 커밋)
>
> ### FE2-1304 — 메이커 유형 판정을 계정 단위로 바로잡았습니다
>
> 어드민이 기획전을 여러 건 한 번에 신청할 때, 신청자가 **신규 메이커인지 기존 메이커인지** 표시합니다.
> 그 판정이 세 가지 문제를 갖고 있었습니다.
>
> | 문제 | 고친 내용 |
> |---|---|
> | 기업번호가 없는 프로젝트 | 기업번호로 찾지 못하면 판정이 비었습니다. 이제 **계정 단위로 판정**합니다 |
> | 조회 실패의 전파 | 메이커 조회가 실패하면 그 실패가 계정 단위 판정까지 흘러갔습니다. 이제 흘리지 않습니다 |
> | 무한 반복 | 사용자 번호로 페이지를 넘기며 조회하는데 **끝나지 않는 경우**가 있었습니다. 순회 횟수에 상한을 뒀습니다 |
>
> 신규 `FundingApiService`(57줄)가 들어왔고, 검증 테스트 `FundingApiServiceTest`(93줄)가 함께 붙었습니다.
> 기업번호가 없는 이유를 적은 주석도 **승인 시점 기준으로** 바로잡았습니다.
>
> ### FE2-1231 — 메이커센터 경로 변경
>
> `wadiz.io/web/maker/makercenter` 경로로 옮기는 작업입니다.
> 프런트엔드 쪽 작업은 [`makercenter-fe`](./makercenter-fe.md) 의 같은 이슈에 있습니다.
