...
> 📅 **2026-09-22 (2차) dev pull 보강** (2 커밋)
>
> 두 커밋 모두 **RWD-5804**(구매확정 후 환불의 adm 이관)입니다.
>
> ### 배송비 지원분 판정을 호출 경로로 갈랐습니다
>
> **종전 주석이 이번에 뒤집혔습니다.** `ShippingRefundPolicy` 에는 이렇게 적혀 있었습니다.
>
> > 단순변심 여부와 배송비 제외 여부를 한 식으로 본다. 두 조건은 원래 따로 쓰였지만 같은 판정으로 합쳐진다.
>
> 이제 **"한 식으로 합칠 수 없다"** 로 바뀌었습니다. 이유가 커밋에 적혀 있습니다.
>
> | 경로 | 무엇으로 판단하나 |
> |---|---|
> | 어드민(`co.wadiz.adm`) | 운영자가 켠 **수수료 면제**와 **배송비 제외** 여부 |
> | 웹(`com.wadiz.web`) | 서포터 신청서의 **단순변심** 여부 |
>
> 한쪽 입력만 살아 있으면 두 식이 같은 값을 냅니다.
> **둘 다 살아 있으면 갈립니다.** 운영자가 배송비 범위를 고른 `REVISED` 신청서가 그 경우입니다.
> 커밋에 **라이브 상한 12,940건**이라고 적혀 있습니다.
>
> 경로를 가르는 신호는 `RefundCommand` 에 새로 생긴 `feeWaivedByOperator` 입니다.
>
> ```java
> // order-core/src/main/java/io/wadiz/order/domain/RefundCommand.java
> Boolean feeWaivedByOperator,   // null 이면 "운영자 화면이 아니다"
> ```
>
> `RefundReason.isFeeWaived()` 와 **뜻은 같고 출처가 다릅니다.**
> 그쪽은 서포터 신청서가 `TRUSTED` 인 것이고, 이쪽은 운영자의 그때 판단입니다.
> 어드민은 체크가 꺼져 있어도 `false` 를 실어 보냅니다. 안 보내면 웹 규칙으로 떨어지기 때문입니다.
>
> 검증 테스트 `ShippingRefundPolicyTest`(+61줄)가 함께 들어왔습니다.
>
> ### 구매확정 후 환불 상태에 마지막 변경자를 실었습니다
>
> `OutOfTermRefundResponse` 에 `actorUserId` 가 생겼습니다.
> **이름이 아니라 번호로 내립니다.** 사람 이름은 `UserProfile` 이 갖고 있고 order 는 그 테이블을 보지 않습니다.
>
> 이력이 없는 캠페인은 `updated` 가 `null` 입니다.
> 실제 행은 SQL 이 언제나 `now()` 를 넣으므로, **이력 유무를 가르는 값이 그것뿐입니다.**
>
> ---

> 📅 **2026-09-22 dev pull 보강** (13 커밋)
>
> 13커밋 전부 `RWD-5804` 입니다. **일괄 취소와 운영자 취소를 다듬는 작업**입니다.
>
> ### 일괄 취소가 중간에 멈추지 않게 했습니다
>
> 일괄 취소는 캠페인이 미달로 끝났을 때 전체 결제를 되돌리는 경로입니다.
> 여러 건을 한 번에 처리하므로 **하나가 걸리면 나머지가 막힙니다.**
>
> | 고친 것 | 내용 |
> |---|---|
> | 이미 반환된 지갑 | 만나도 **멈추지 않고 넘어갑니다** |
> | 되돌릴 것이 없는 건 | 오류로 보지 않고 **통과시킵니다** |
> | 금액 정책 | 일괄 취소는 **전액 정책**을 쓰도록 했습니다 |
>
> ### 가드를 취소 종류별로 좁혔습니다
>
> 발송 여부와 최종 결제일 마감을 확인하는 가드가 있습니다.
> 이 가드를 **서포터가 스스로 하는 취소에만** 걸도록 범위를 좁혔습니다.
> 두 번에 걸쳐 조정했습니다. 먼저 주문취소 전용으로 되돌리고, 다시 서포터 취소 전용으로 좁혔습니다.
>
> 검증 테스트 `CancelOnlyGuardTest`(145줄)와 `NothingToRefundTest`(24줄)가 새로 붙었습니다.
>
> ### 운영자 단건 취소에 전용 알림톡
>
> - 운영자가 한 건씩 취소할 때 **전용 알림톡 템플릿 3232** 를 씁니다.
> - 링크 치환자를 바로잡고 **취소 메일 1543·1544 를 이관**했습니다.
> - `kr-payment` 에 `cancelRequester` 를 **열거형 이름 그대로** 보냅니다. 계약 테스트 `CancelRequesterContractTest`(104줄) 신설.
>
> ### 정합성 수정 네 건
>
> | 고친 것 | 내용 |
> |---|---|
> | 지갑 잔여 계산 | 플래그가 아니라 **배분 누계로** 셉니다 |
> | 쿠폰 선점 | `IsRefund` 가 비어 있으면 거짓으로 봅니다 |
> | 환율 조회 | `BackingPaymentFx` 를 **한 행만** 읽습니다 |
> | 신청 커밋 | 신청은 `APPLY` 로 커밋하고, **집행이 끝난 회차만** 확정합니다 |
>
> ### ⚠️ PG 중복 호출 방지가 하나 늘었습니다
>
> **결과를 확인하지 못한 시도가 남아 있으면 PG 를 다시 부르지 않습니다.**
> 이 문서의 "실패 세 갈래" 중 `PgOutcomeUnknownException` 과 이어지는 장치입니다.
> 갔는지 모르는 상태에서 다시 부르면 이중 결제 취소가 됩니다.
>
> ### 배포
>
> `rc1` 배포 워크플로를 추가했습니다. [`helm-charts`](./helm-charts.md) 의 rc1 전면 복원과 맞춘 것입니다.

> 📅 **2026-09-17 dev pull 보강** (4 커밋)
>
> 네 커밋 모두 `RWD-5804` 이고, **레거시와 동작을 맞추는 정리**입니다.
>
> | 커밋 | 내용 |
> |---|---|
> | 지갑 잔여 판정 | 지갑 잔여를 계산할 때 **`IsRefund` 값을 보도록** 고쳤습니다 (`RefundContextMapper.xml`) |
> | 요청 구조 정리 | **서버가 이미 아는 값을 요청에서 뺐습니다.** 보류 예정일 계산도 서버로 옮겼습니다. 신규 `BusinessDayMapper.xml` 이 들어왔습니다 |
> | 오류 코드 | **레거시가 내보내던 오류 코드 문자열을 그대로 씁니다.** 검증 테스트 `LegacyErrorCodeTest`(34줄)가 함께 붙었습니다 |
> | 배송비 | 미리보기 응답에 **결제 시점의 배송비**를 싣습니다. 검증 테스트 `PaidShippingTest`(53줄) 신규 |
>
> 오류 코드를 레거시 문자열로 맞춘 것이 눈에 띕니다.
> 이 서비스는 [`com.wadiz.web`](./com.wadiz.web.md)·[`co.wadiz.adm`](./co.wadiz.adm.md) 과 같은 테이블을 쓰는 과도기입니다.
> 응답 문자열까지 맞춰야 기존 화면이 그대로 동작합니다.
>
> 테스트가 계속 늘고 있습니다. 이번에 `RefundCommandFillTest`(63줄)도 추가됐습니다.
...
| `dev` | dev | `core/dev/order-api.yaml` | `dev-202609221302-53891100` (2026-09-22 확인) |
