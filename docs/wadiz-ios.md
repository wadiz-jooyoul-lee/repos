...
> **`kr` 도메인 단일화(FE1-1762, 10커밋)** 와 **서포터클럽 배너 공용화(FE1-1626, 10커밋)** 가 뒤를 잇습니다.
>
> ### FE1-1848 — 막펀잡기 카드 레이아웃 A/B 실험 (32커밋)
>
> 막펀잡기는 마감 임박 프로젝트를 카드로 넘겨 보는 화면입니다.
> **어떤 버튼 배치가 더 잘 눌리는지**를 두 안으로 나눠 실험합니다.
>
> | 안 | 버튼 구성 |
> |---|---|
> | 그룹 A | `[다음 카드보기]` + `[상세보기]` 두 개 |
> | 그룹 B | 5대5 폭, 라벨을 `[펀딩하기]` 로 고정 |
>
> 실험 키는 `endingsoon_cta_position` 이고 값은 데이터 시트에서 받습니다.
>
> **측정에서 결함 하나를 고쳤습니다.**
> 종전에는 **앱을 켜기만 해도 노출이 찍혔습니다.** 이제 **그 지면이 실제로 보일 때** 기록합니다.
> 그리고 실험 배정값을 전 이벤트에 자동으로 붙입니다.
>
> 레이아웃 작업은 T1 부터 T14 까지 나눠 진행했습니다.
>
> - 상단 예약 영역 130, 이미지 높이 축소 산식(T1)
> - 진행바 offset 24 를 카드 소속으로, 타이머 pill 배치(T2)
> - 이미지 좌우 16 여백, 공간이 모자라면 이미지를 줄임(T3)
> - 뱃지 행은 하나만. **찜이 있으면 찜 우선**, 없으면 우선순위 첫 활동배너(T4)
> - 쿠폰 금액 뱃지는 **다운로드 후에도 유지**(T5)
> - 타이머 pill 을 카드 소속으로 바꿔 카드와 함께 스크롤(T14)
>
> 실험 식별 방식도 정리했습니다.
> 고정 이름 필드를 걷어내고 **동적 파라미터 하나로** 실험을 가립니다.
> 리뷰에서 실험이 꺼졌을 때의 폴백 순서를 계약 테스트로 고정했습니다.
>
> ### FE1-1762 — `kr` 도메인 단일화 (10커밋)
>
> 안드로이드의 `FE1-1760` 과 같은 작업입니다.
>
> - 환경 정의와 **바깥으로 나가는 도메인을 하나로** 합쳤습니다(T1).
> - **들어오는 `kr` 주소는 그대로 받습니다.** 판정만 제거하고 가드를 보정했습니다(T2).
> - 클라우드 전환용 인프라 코드를 걷어냈습니다.
> - 시험·목·미리보기·개발도구의 `kr` 도메인도 전환했습니다.
> - `ai` 도메인 링크가 잘못 분류되던 것을 되살렸습니다.
>
> ### FE1-1626 — 서포터클럽 배너를 공용 부품으로
>
> - `Core/UI` 에 **서포터클럽 배너 컴포넌트**를 만들고, `Model` 에 상태·노출 판정을 넣었습니다.
> - 마이와디즈 배너와 혜택 홈 배너를 **같은 판정과 같은 컴포넌트로** 바꿨습니다.
> - **마이와디즈 API 를 v6 로** 올리고 `graceUntil`(유예 종료일)을 배선했습니다.
> - 서포터클럽 표기 기준을 안드로이드와 맞췄습니다.
> - ⚠️ **혜택 홈에서 쿠폰 조회가 실패하면 화면 전체가 막히던 문제**를 고쳤습니다.
>
> ### 그 밖에
>
> | 이슈 | 내용 |
> |---|---|
> | FE1-1833 | `navigation-map` 의 막펀잡기 기본값에 `links.global` 을 추가하고, 글로벌·국내 라우트 검증 테스트를 붙였습니다. 정본 Mock `NavigatorTesting` 도 신설했습니다 |
> | FE1-1522 | 최초 실행 시 로케일 초기화를 **권한 화면보다 먼저** 시작합니다. 권한 화면의 PV 는 안드로이드와 맞춰 제거했습니다 |
> | SDQA-5 · 7 · 8 · 10 | 디자인 검수 반영 |

> 📅 **2026-09-10 main pull 보강** (61 커밋, 26.36.1.0 → **26.37.0.12**)
>
> **막펀잡기 알림(FE1-1638, 25커밋 + FE1-1821, 7커밋)** 한 덩어리가 대부분입니다. 변경 파일 120개 중 56개가 `Projects/Features/EndingSoon` 입니다. (Bump up·다국어 동기화 커밋 생략)
>
> ### FE1-1638 — 막펀잡기 알림 수신동의와 유도 모달 (25커밋)
> - **알림 설정 화면**(목록 항목 + 네이티브 페이지)과 **알림 수신 유도 모달 3종**을 만들었습니다. 수신동의는 서버(CRM API)와 연동합니다 — [`kr.wadiz.platform.crm`](./display-platform-services-3-messaging.md) 의 DISPLAY-1702 이고, 인프라 쪽 허용은 [`helm-charts`](./helm-charts.md) CINFRA-665 입니다.
> - **노출 정책(T2)** 을 따로 설계했습니다 — 판정·영속·세션·A/B 네 축입니다. 즉 "언제 누구에게 몇 번 보여줄지" 를 정책으로 분리했습니다.
> - 화면 구현으로 **타이틀 지우개 뷰**와 **플립 카운트다운 뷰**, 모달 애니메이션 커브가 들어왔습니다. 유도 모달이 떠 있는 동안 **피드 자동넘김을 정지**합니다.
> - 개발자도구에 **알림 모달 미리보기**를 붙였고, A/B 실험 key 는 서버 등록값을 반영했습니다.
> - `.claude/harness/FE1-1638/` 에 spec 정제·사인오프·task_list 5건이 저장소에 함께 들어왔습니다.
> - PR 리뷰에서 **권한 콜백 누락과 조회 실패 은폐**가 지적돼 대응했습니다.
> - 안드로이드의 짝은 FE1-1648·1649 입니다.
>
> ### FE1-1821 — 막펀잡기 알림 후속 정비 (7커밋)
> - **복귀 유도를 거절하면 7일 쿨다운**을 둡니다. **카운트다운 대상이 없으면 복귀 유도를 띄우지 않습니다.**
> - **일시정지 중에도 마감 카운트다운은 계속 진행**합니다.
> - 수신동의 저장 응답의 **`message` 로 성공을 판정하던 것을 제거**하고, 저장값이 요청과 다를 때 **로그를 남깁니다**.
> - 알림 설정 페이지 PV 발화 추가, spec·as-built 문서 갱신.
>
> ### 기타
>
> | 이슈 | 내용 |
> |---|---|
> | FE1-1371 | 와디태그 수집 정비 — 스토어홈 URL 정규화 · **`hashedUserName` 을 Keychain 에 영속화** · 펀딩류의 `price`/`discountRate` 를 0 으로 전송. 안드로이드 FE1-1370 과 짝 |
> | FE1-1825 | **찜 마감임박(endingsoon) 호출을 `wish` 도메인에서 `searcher` 도메인으로 옮겼습니다** — `WishAPIImpl` 의 `APIURLSource` 를 `.service(.wish)` → `.service(.searcher)` 로 바꾸고, cloud 정책의 컨텍스트를 `/wish` → `/search` 로 뒀습니다. 새 `searcher` case 는 `/api/search` 가 아닌 **`/api/v1/searcher/**` 경로용**입니다.<br>⚠️ **웹은 같은 시기에 정반대로 갔습니다** — `wadiz-frontend` FE1-1826 은 찜 마감임박 조회를 **검색 서버에서 `wish-api` 로 전환**했습니다. 두 클라이언트가 같은 데이터를 서로 다른 서버에서 읽게 된 셈이라 **의도된 것인지 확인이 필요합니다.** |
> | FE1-1822 | 검색 화면 전환·검색 실행 시 키보드 해제, 검색결과 복귀 시 피드 영상 재생 재개 |
> | FE1-1793 / FE1-1794 | UITest 환경을 RC → **RC4** 로 변경, 개발자도구에 **A/B 테스트 지정 기능** 추가 |
> | FE1-1472 | 머지백 PR 의 head 를 `main` 에서 **전용 브랜치로 변경** |
> | QA-23288 | 비로그인 진입 후 로그인 시 최초 모달이 뜨지 않던 문제 수정 |
>
> ---

> 📅 **2026-09-08 main pull 보강** (2 커밋, → 26.36.1.0)
>
> ### FE1-1806 — 페이스북 SDK 18.0.1 → 18.1.1
> - 페이스북 SDK 를 올렸습니다(`Tuist/Package.swift`·`Package.resolved`·`Projects/App`). 웹의 FE1-1808(아이폰 웹 페이스북 공유 문제)과 같은 시기이나 별개 이슈입니다.
>
> ---
