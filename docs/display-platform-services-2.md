...
> 📅 **2026-09-22 main pull 보강** — `wish-api` (7 커밋)
>
> ### DISPLAY-1757 — 마감임박 찜 프로젝트 조회 개선
>
> - ⚠️ **이미 마감된 프로젝트가 마감임박 목록에 섞여 나오던 문제**를 고쳤습니다.
> - 마감임박 목록의 **개수만 세는 API** 를 따로 만들었습니다.
> - 마감임박 id 목록 **캐시 유효 시간을 3분으로 줄였습니다.** 마감이 임박한 목록이라 오래 두면 어긋납니다.
>
> ### DISPLAY-1782 — 찜한 프로젝트 조회를 커서 페이징으로
>
> - 페이지 번호 대신 **커서 방식**으로 바꿨습니다. 목록이 바뀌어도 건너뛰거나 겹치지 않습니다.
> - ⚠️ **N+1 조회를 없앴습니다.** 목록을 가져온 뒤 건마다 다시 조회하던 구조였습니다.
> - 페이징 정보를 나르는 객체를 공용 `PagedResult` 로 합치고, 커서 정보를 응답 봉투로 옮겼습니다.
>
> ### DISPLAY-1775 — 스토어 할인 프로젝트 조회 정리
>
> 응답을 DTO 로 바꾸고 `projectType` 을 뺐습니다. 신규 `WishProductType` enum 이 들어왔습니다.
