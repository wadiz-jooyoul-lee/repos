...
> 📅 **2026-09-17 main pull 보강** (15 커밋)
>
> 15커밋 전부 `DISPLAY-1726` 이고, **브랜드 메시지의 "카탈로그형" 지원**이 주제입니다.
>
> 브랜드 메시지는 카카오톡으로 보내는 마케팅 메시지입니다.
> 카탈로그형은 그중 **상품 카드를 여러 장 늘어놓는 형식**입니다.
>
> ### 카탈로그형 브랜드 메시지 템플릿 관리
>
> | 변경 | 내용 |
> |---|---|
> | 카탈로그형 템플릿 | 카탈로그형(`CATALOG`) 브랜드 메시지 템플릿을 어드민에서 만들 수 있게 했습니다 |
> | 버튼 개수 제한 | 브랜드 메시지 **타입별로 버튼 개수 상한**을 뒀습니다 |
> | 미사용 필드 제거 | 템플릿의 `title` 필드를 지웠습니다 |
> | 대체문자 관리 | 브랜드 메시지에 **대체문자 필드와 대체발송 지원 여부**를 관리하도록 추가했습니다. 대체문자는 카카오톡 발송이 실패했을 때 문자로 보내는 대체 수단입니다 |
> | 대체문자 검증 위치 | 필수 검증을 화면이 아니라 **서비스 검증 로직으로** 옮겼습니다 |
> | 카탈로그형 대체문자 | 카탈로그형에도 대체문자 서브템플릿을 등록할 수 있게 했습니다 |
> | 네이밍 정리 | 대체문자 아이템의 템플릿 필드와 치환자 이름을 정리했습니다 |
>
> 마지막 커밋에서 **치환자 필드의 글자수 검증을 제거**했습니다.
> 치환자는 발송 시점에 실제 값으로 바뀌는 자리표시자라, 등록 시점의 글자수로 판단할 수 없기 때문으로 보입니다(추정입니다. 커밋 제목과 치환자의 성격으로 판단했습니다).
>
> ### 도메인코드 관리 화면과 환경 동기화 (5커밋)
>
> 도메인코드는 메시지가 어느 서비스 영역에 속하는지 구분하는 코드입니다.
> 2026-08-27 에 `DISPLAY-1723` 으로 조회를 이 서비스로 모았고, 이번에 **관리 화면**을 붙였습니다.
>
> - 신규 `static/assets/js/common/domainCodeManage.js`(451줄) — 도메인코드를 화면에서 관리합니다.
> - **환경 동기화** 기능을 넣었습니다. 한 환경에서 만든 도메인코드를 다른 환경으로 옮깁니다.
> - 동기화 과정에서 세 가지를 고쳤습니다.
>   - 응답을 역직렬화하다 나던 오류
>   - 대상 환경에 없을 때 **신규 등록으로 처리**하지 못하던 문제
>   - 존재 확인 조건이 복잡하던 것을 단순화
> - 관리 API 를 정리하고 조회를 하나로 합쳤습니다.
>
> ### 그 밖에
>
> 메일 템플릿 공개 화면에서 **치환자가 처음에 표시되지 않던 문제**를 고쳤습니다.
>
> ### 짝이 되는 저장소
>
> 발송 쪽 구현은 `kr.wadiz.platform.api.friendtalk` 에 있습니다.
> 같은 `DISPLAY-1726` 으로 비율별 이미지 업로드와 카탈로그형 발송이 함께 만들어졌습니다.
> [묶음 ③ 발송 채널 문서](./display-platform-services-3-messaging.md)를 참고하세요.
