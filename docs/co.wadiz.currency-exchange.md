...
> 📅 **2026-09-15 main pull 보강** (2 커밋)
>
> ### 배포 워크플로를 하나로 합쳤습니다
> - 환경별로 나뉘어 있던 워크플로 4개(`dev`·`live`·`rc`·`rc2`)를 **브랜치로 대상을 가르는 단일 워크플로 `aws_deploy_ecr.yml`(51줄)** 로 통합했습니다.
> - 그 전에 **odev·rc1·rc2·live values 갱신을 먼저 제거**했습니다 — 이 저장소의 원격 `cloud_live` 가 삭제된 것과 같은 맥락의 온프레미스 정리입니다.
> - 같은 형태의 정리가 `kr.wadiz.backoffice.indexer-schedule` 에도 같은 날 적용됐습니다(문서 없음).
