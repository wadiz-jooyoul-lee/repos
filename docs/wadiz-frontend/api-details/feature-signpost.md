...
> **범위**: `apps/global` + `packages` / **단위**: 기능·컴포넌트
> **경로 표기**: 모두 repo 루트(`wadiz-frontend`) 기준 상대경로.

## 사용법

- `Ctrl+F`로 **기능명 · 화면 문구 · 이슈키(FE1-xxxx)** 를 검색해 소스 위치를 찾습니다.
- 문구는 대부분 `@wadiz/i18n`의 `useTranslation({ keyPrefix })` + `t('상대키')` 조합입니다. 실제 텍스트는 `packages/i18n/src/supporter/languages/{ko,en,ja,zh}.json` 의 `keyPrefix` 네임스페이스 아래에서 확인합니다.
- 특정 문구가 하드코딩(비 i18n)인 경우 해당 표에 별도 표기했습니다.
- 섹션 하단의 **이슈 히스토리** 표는 그 도메인을 건드린 Jira 이슈 제목 모음입니다. 이슈 제목으로 관련 영역을 먼저 좁힌 뒤 상단 표에서 소스를 찾습니다.

## 도메인 인덱스

| 도메인 | 상태 |
|---|---|
| [펀딩 상세 (Funding Detail)](#펀딩-상세-funding-detail) | 완료 |
| [결제 (Funding Payment)](#결제-funding-payment) | 완료 |
| [홈 / 서비스홈 (Home / Service-Home)](#홈--서비스홈-home--service-home) | 완료 |
| [마이와디즈 / 위시 (My-Wadiz / Wish)](#마이와디즈--위시-my-wadiz--wish) | 완료 |
| [메이커 / 프로젝트 만들기 (Maker / Create-Project)](#메이커--프로젝트-만들기-maker--create-project) | 완료 |
| [소셜 / 친구추천 (Social / Refer-a-Friend)](#소셜--친구추천-social--refer-a-friend) | 완료 |
| [검색 / 스토어 / 소싱클럽 (Search / Store / Sourcing-Club)](#검색--스토어--소싱클럽-search--store--sourcing-club) | 완료 |
| [정책 / WAi / 알림 / 이벤트 / 쿠폰 / 기타](#정책--wai--알림--이벤트--쿠폰--기타) | 완료 |
| [packages 공통 (ui · widgets · features)](#packages-공통-ui--widgets--features) | 완료 |
