...
>
> 세 가지가 큽니다.
> **이미지 최적화 모듈 통합**(FE1-1830, 24커밋), **스튜디오 서비스 요금 화면 개편**(FE2-1297, 14커밋),
> **스튜디오 자동 검사 안정화**(FE2-1174, 10커밋)입니다.
>
> ---
>
> ## 1. FE1-1830 — 이미지 최적화를 한 모듈로 모으고 스위치를 달았습니다
>
> 2026-09-10 회차에 `OptimizedImage` 로 컴포넌트를 합쳤습니다. 이번에는 **변환 로직 자체**를 모았습니다.
>
> | 변경 | 내용 |
> |---|---|
> | 모듈 신설 | `image-optimization` 을 만들고 변환 본체를 옮겼습니다. 형식 기본값은 webp 입니다 |
> | 컴포넌트 통합 | 이미지 최적화를 `ui` 로 옮겨 `OptimizedImage` 와 합쳤습니다 |
> | 판정 분리 | CDN 호스트 지도와 변환 가능 판정을 `url.js` 에서 떼어냈습니다 |
> | 함수 통합 | 흩어져 있던 이미지 최적화 함수를 하나로 합치고 호출부를 옮겼습니다 |
>
> **webp 승격을 끌 수 있게 했습니다.**
>
> - 설정으로 webp 승격을 **전체적으로 끌 수 있습니다.** 스위치 판정은 앱을 띄울 때 주입합니다.
> - **특정 프로젝트의 스토리만 webp 에서 뺄 수도 있습니다.**
> - 스토어 상세도 이 스위치를 따릅니다.
>
> 이 스위치는 webp 변환에서 생긴 문제 때문으로 보입니다. 다만 **어떤 문제인지 커밋에는 적혀 있지 않습니다.**
> 특정 프로젝트만 빼는 기능이 있는 점에서 개별 사례 대응으로 보이나, 이 판단은 추측입니다.
>
> 크기가 고정된 세 자리와 오픈예정 카드는 **표시 크기를 직접 넘기도록** 바꿨습니다.
> 리뷰가 6차까지 갔습니다.
>
> ---
>
> ## 2. FE2-1297 — 스튜디오 서비스 요금 화면 개편 (14커밋)
>
> 메이커가 요금제와 컨설팅을 고르는 화면입니다.
>
> | 변경 | 내용 |
> |---|---|
> | 신청 방식 | 컨설팅 서비스 신청 영역을 **신청·미신청 선택형**으로 바꿨습니다 |
> | 상태 안내 | 컨설팅 승인·반려 상태별로 안내를 나눠 보여 줍니다 |
> | 결과 요약 | 저장 후 **결과 요약 모달**을 띄웁니다. 모바일 우선 반응형입니다 |
> | 안내 구조 | 요금제 안내를 **조건·설명·가이드 링크** 세 부분으로 나눴습니다 |
> | 수수료 카드 | 항목 번호를 해외·국내 요금 카드 수에 맞춰 나눕니다 |
> | 레거시 정리 | 펀딩 플랜 옛 화면을 `deprecated` 디렉터리로 옮기고 남은 참조를 지웠습니다 |
>
> 폴백을 둔 곳이 둘 있습니다.
> 요금제 라벨 키가 없으면 `STANDARD` 로 떨어지고, 컨설팅 신청 여부 기본값은 저장값과 제출 여부를 보고 정합니다.
>
> 백엔드 짝은 [`co.wadiz.adm`](../co.wadiz.adm.md) 의 RWD-5954(PD 컨설팅 V3)입니다.
>
> ---
>
> ## 3. FE2-1174 — 스튜디오 자동 검사 오탐 제거 (10커밋)
>
> 자동 검사가 **코드 문제가 아닌 이유로 실패하던 것**을 걷어냈습니다.
>
> - 화면 진입을 **재시도 헬퍼로 통일**하고 전제 조회에 시간 제한을 뒀습니다.
> - 일시적 네트워크 오류 코드를 재시도 대상에 넣었습니다.
> - 오류 화면 판정에 `serverError` 를 더하고, 재시도 후에도 오류면 건너뜁니다.
> - ⚠️ 다만 **렌더 크래시 경로는 건너뛰기 대상에서 뺐습니다.** 진짜 결함을 놓치지 않으려는 구분입니다.
> - 건너뛴 경우 **왜 건너뛰었는지를 주석과 메시지에 적습니다.**
> - 보류했던 새소식 인수조건을 되살렸습니다.
>
> ---
>
> ## 4. FE1-1939 — 새소식 상세에 서포터클럽 가입 유도 배너
>
> - 오픈예정 새소식 상세에 가입 유도 배너를 띄웁니다. **노출 대상은 오픈예정 알림신청자로 한정**합니다.
> - 배너가 뜨는 동안 **LiveActivity 배너를 감춥니다.** 처음에는 floating 만 막았다가 완전 비노출로 바꿨습니다.
> - 배너 높이를 **요소 높이 추적기에 등록**해 하단 여백과 떠 있는 버튼 위치에 반영합니다.
> - 배너 노출 상태 전달을 전역 저장소에서 **Outlet context 로** 바꿨습니다.
>
> ---
>
> ## 5. 도메인·정적 자원 정리
...
> | 이슈 | 내용 |
> |---|---|
> | FE1-1944 | 남아 있던 `wadiz.kr` 호스트를 `wadiz.io` 로 치환했습니다. 도메인 전환으로 어긋난 판별 로직과 프록시 매핑도 고쳤습니다 |
> | CLIENT-274 | 정적 자원 주소를 `cdn-static` 주소로 교체했습니다 |
> | CLIENT-272 | 빌드 설정에서 `SpeedMeasurePlugin` 을 뺐습니다 |
>
> ---
>
> ## 6. 브레이즈 정리 마무리
>
> 2026-09-17 회차에 시작한 작업의 끝입니다.
>
> - **`appboy` 전역 직접 접근을 ESLint 로 차단**했습니다(FE1-1894). 규칙으로 막아 다시 생기지 않게 했습니다.
> - 스튜디오의 브레이즈 인라인 스크립트 주입을 제거했습니다(FE1-1954).
>
> ---
>
> ## 7. 수집 정합성 (FE1-1959 · FE1-1968)
>
> - **노출과 클릭의 `tagUrl` 이 달라지던 문제**를 고쳤습니다. 피드 카드의 랜딩 주소 조립을 한 곳으로 모았습니다.
> - 전자상거래 노출 선언을 **노출 전용 링크로 분리**하고 `ref`·`uid` 로 연결했습니다.
> - 속성 펼침 순서를 바꿔 **훅이 보장하는 속성이 덮이지 않도록** 했습니다.
> - 와디태그 v2 전송에 `catch` 를 붙여 처리되지 않은 rejection 을 없앴습니다.
>
> ---
>
> ## 8. 그 밖에
...
> | 이슈 | 내용 |
> |---|---|
> | FE2-1334 | 메이커홈과 모바일 WAi 에 **스토리 결과보기 새 탭 미리보기**를 넣었습니다. 오버레이 z-index 를 모달 아래로 낮추고 뒤로가기로 닫히게 했습니다 |
> | CLIENT-262 · CLIENT-263 | 번들 언어 불일치 보고를 **warning 으로 낮추고 경우별로 묶었습니다.** 쿠키 스냅샷을 만든 시각도 함께 기록합니다. 센트리 래퍼가 `fingerprint` 를 전달하도록 고쳤습니다 |
> | FE1-1940 | 도달할 수 없는 오픈예정 새소식 레거시 패키지를 지웠습니다 |

> 📅 **2026-09-17 cloud_live pull 보강** (111 커밋, 274파일 +5,530/−1,643)
>
> 세 가지가 나란히 큽니다.
> 첫째는 **앱 설정 콘솔 어드민 화면 신설**(FE1-1854·FE1-1858, 32커밋)입니다.
> 둘째는 **브레이즈 수집 코드를 프런트엔드로 흡수**(FE1-1874 계열, 18커밋)하는 작업입니다.
> 셋째는 **인공지능 심의 결과 화면**(FE2-1176, 11커밋)입니다.
>
> ---
>
> ## 1. 앱 설정 콘솔 어드민 화면 (FE1-1854 · FE1-1858 · FE1-1860, 33커밋)
>
> 앱(아이폰·안드로이드)의 설정값을 웹 화면에서 편집하는 어드민입니다.
> 종전에는 [`co.wadiz.adm`](../co.wadiz.adm.md) 이 중계했는데, **이제 화면이 `app-api` 를 직접 부릅니다.**
>
> ### FE1-1854 — 설정 조회·편집 화면
>
> | 기능 | 내용 |
> |---|---|
> | 직접 호출 전환 | 앱 설정 API 를 `app-api` 로 직접 호출합니다. 어드민 중계를 걷어냈습니다 |
> | 쓰기 권한 | 쓰기 요청에 **환경별 토큰**을 붙입니다 |
> | 주소·헤더 평가 시점 | 모듈이 읽히는 시점이 아니라 **호출하는 시점에** 주소와 인증 헤더를 계산합니다 |
> | 설정 카드 | 기능 이름과 설정 값을 **복사하는 버튼**을 붙였습니다 |
>
> ### FE1-1858 — 앱 운영 항목 "편성" 화면 (22커밋)
>
> 앱 홈에 무엇을 언제 보여 줄지 정하는 화면입니다. 표 형태로 항목을 편집합니다.
>
> - **표 기능**: 정렬, 필터, 검색, 노출 기간 필터, 표시 개수 선택, 페이지 나누기를 모두 붙였습니다.
> - **미리보기**: 편성에 올린 이미지·영상·로띠(Lottie, 가벼운 애니메이션 형식) 파일을 화면에서 바로 봅니다.
> - **색 지정**: 배경색 견본을 보여 주고 색 선택기를 띄웁니다.
> - **실수 방지 세 가지**
>   - 삭제 버튼을 누를 때만 체크박스가 나타납니다. 평소에는 보이지 않습니다.
>   - 저장 직전에 운영 값의 형식을 검증합니다.
>   - 저장 전 확인창을 띄우고, 노출 스위치를 끌 때도 한 번 더 확인합니다.
> - **저장 보호**: 편성 저장이 **기존 설정 구조를 바꾸지 않도록** 막았습니다.
> - 업로드한 파일 주소는 **환경별 CDN 도메인**으로 저장합니다. CDN 은 파일을 각지 서버에 복사해 두는 전송망입니다.
> - 업로드가 실패하면 **닫아야 사라지는 알림**으로 안내합니다. 자동으로 사라지지 않습니다.
>
> ---
>
> ## 2. 브레이즈 수집을 프런트엔드로 흡수 (FE1-1874 · 1891 · 1892 · 1893 · 1896 · 1446, 18커밋)
>
> 브레이즈는 사용자 행동을 모아 푸시·메일을 보내는 마케팅 도구입니다.
> 종전에는 [`com.wadiz.web`](../com.wadiz.web.md) 의 JSP 가 브레이즈 SDK 를 직접 불러 초기화했습니다.
> 이번에 그 역할을 **프런트엔드의 `wadiz.braze` 모듈 하나로 모았습니다.**
>
> | 변경 | 내용 |
> |---|---|
> | 진입점 통합 | JSP 는 함수 이름만 부르고, 구현은 프런트엔드 번들이 등록합니다 |
> | 중복 로드 방지 | **셸이 이미 SDK 를 내려받고 있으면 다시 받지 않습니다.** 도착 여부는 대역 함수의 흔적으로 판정합니다 |
> | 앱 웹뷰 | 앱 안에서는 웹 SDK 대신 **브리지로 전환**하고 속성 이름을 v2 로 바꿨습니다 |
> | 국내 비로그인 | 초기화하지 않습니다. 이 제한이 **웹에만 해당한다는 점**을 주석에 명시했습니다 |
> | 중복 초기화 제거 | `account` 앱이 따로 초기화하던 것을 없앴다가, 필요해서 다시 살렸습니다 |
> | 스튜디오 | 자체 초기화를 없애고 수집 모듈로 넘겼습니다. 쓰이지 않게 된 API 키 import 도 지웠습니다 |
> | 로드 실패 | SDK 스크립트 로드에 실패하면 콘솔에 남깁니다 |
>
> ---
>
> ## 3. 인공지능 심의 결과 화면 (FE2-1176, 11커밋)
>
> 메이커가 스튜디오에서 자기 프로젝트의 인공지능 심사 결과를 보는 화면입니다.
>
> - **심사 유형을 구분해 보여 줍니다.** 자동 심사와 요청 심사의 안내 문구가 다르고, 자동 심사에는 툴팁을 붙였습니다.
> - **종료된 프로젝트도 결과를 조회**할 수 있게 했습니다. 결과가 있는지에 따라 사이드바 노출과 언어 선택을 갈랐습니다.
> - 심사 상태에 따라 **스토리 저장을 제어**합니다.
> - 인공지능 심사 결과 메일 템플릿에 **다국어 제목**을 넣고 템플릿 번호를 정정했습니다.
> - 조회 쿼리 파라미터 이름을 케밥 케이스(단어를 붙임표로 잇는 표기)로 고쳤습니다.
> - 백엔드 짝은 [`com.wadiz.api.funding`](../com.wadiz.api.funding/com.wadiz.api.funding.md)·[`co.wadiz.adm`](../co.wadiz.adm.md) 의 RWD-5975 입니다.
>
> ---
>
> ## 4. 온프레미스 흔적 마저 제거 (CLIENT-259, 11커밋)
>
> 직전 회차에 환경 목록을 클라우드 5종으로 정리한 작업의 뒷정리입니다.
>
> - **항상 참이 된 `isCloudEnv` 판정을 지웠습니다.** 온프레미스가 없어져 분기가 의미를 잃었기 때문입니다.
> - 어드민 실행 환경 목록에서 없어진 `rc`·`rc2` 를 뺐습니다.
> - 어드민 호스트 판정을 **배포 호스트 표 하나** 기준으로 통일했습니다.
> - 위젯에 남아 있던 `wadiz.kr` 분기와 온프레미스 로컬 주소 판별을 지웠습니다.
> - live 전용 조건에 `clive` 환경을 추가했습니다.
>
> ---
>
> ## 5. 노출·클릭 수집 방식 정리 (FE1-1838 · 1839 · 1852 · 1912 · 1941)
>
> 어떤 카드가 화면에 보였고 어디를 눌렀는지 모으는 방식을 바꿨습니다.
>
> - **카드 루트 선언과 `click-id` 하나로 단일화**했습니다. 기획전 카드·지지서명 상세·친구 피드에 차례로 적용했습니다.
> - 클릭 리스트 이름을 **노출 리스트 이름과 `click-id` 꼬리표로 연결**하는 방식으로 바꿨습니다.
> - **노출로 인정하는 시간 기준을 "누적 1초" 에서 "연속 1초" 로** 바꿨습니다(FE1-1912). 스크롤로 스쳐 간 카드가 노출로 잡히던 문제로 보입니다(추정입니다. 커밋 제목과 기준 변경 방향으로 판단했습니다).
> - `tagid` 가 없는 상황에 가드를 넣어 수집이 깨지지 않게 했습니다(FE1-1941).
>
> ---
>
> ## 6. 그 밖에
...
> | 이슈 | 내용 |
> |---|---|
> | FE1-1755 | 혜택 안내 랜딩을 `apps/global` 한국 라우트로 이관하고 다국어를 적용했습니다. 랜딩 이미지는 CDN 자산으로 바꿨습니다 |
> | FE1-1936 | 오픈예정 상세의 혜택 지면에 **서포터클럽 가입 유도 배너**를 띄웁니다. 배너 문구는 오픈알림 소구로 교체했고, 글로벌 지면에서는 멤버십 조회가 돌지 않게 막았습니다 |
> | FE1-1918 | 참여 취소 메일 V2 템플릿을 추가하고, 리워드 영역을 **복수 리워드 HTML** 로 교체했습니다 |
> | FE1-1923 | 데스크탑 레이아웃에서 **인앱 SPA 로드 완료 이벤트가 전송되지 않던 문제**를 고쳤습니다 |
> | FE1-1926 | 통합 기획전의 디버그 로그가 라이브 환경에 보이던 것을 감췄습니다 |
> | CLIENT-273 | 생성형 UI 의 수집 라벨을 **등록 문구와 표시 문구 기준**으로 바꾸고 라벨 보관소를 만들었습니다 |
> | FE2-1096 | 스토어 어드민 상세에서 프로젝트 번호를 API 가 아니라 **주소 경로에서** 읽도록 바꿨습니다 |
> | SDQA-13 · SDQA-14 | 데스크탑 혜택 라벨 폭, 서포터클럽 배너 로고와 텍스트 간격(8px) 수정 |

> 📅 **2026-09-15 cloud_live pull 보강** (104 커밋, 358파일 +7,464/−4,236)
>
> ⚠️ **직전 동기화 뒤 로컬이 `cloud_dev` 로 옮겨져 있었고 그 원격 브랜치가 삭제돼 pull 이 실패했습니다.** `cloud_live` 로 되돌린 뒤 2026-09-10 시점(`507c0eab7cf`)부터 다시 맞췄습니다.
>
> **온프레미스 환경 제거(CLIENT-259)** 와 **새소식 예약 발행 UI(FE2-1255)** 가 두 축입니다.
>
> ### 🚩 CLIENT-259 — 온프레미스 환경을 코드에서 걷어냈습니다 (11커밋)
> - **`Environment` 타입과 환경별 도메인을 클라우드 5종으로 정리**했습니다. 온프레미스 환경 파일을 제거하고 **`cdev` 설정을 `dev` 로 이동**했습니다(이제 `dev` 가 곧 클라우드 dev).
> - `package.json` 의 온프레미스 빌드·실행 스크립트, 빌드·배포 스크립트의 온프레미스 분기, 워크플로의 온프레미스 선택지와 브랜치 분기를 모두 지웠습니다.
> - **어드민 호스트 판정에서 온프레미스 도메인 분기를 제거**하고 메이커센터 호스트를 일원화했습니다.
> - 환경 기본 주소와 인증 헤더를 **호출 시점 함수로 통일**했습니다(모듈 로드 시점 상수에서 전환).
> - dev 배포가 `wadiz.static.cdev` 버킷을 바라보도록 교체했습니다.
>
> ### CLIENT-258 — stage 환경을 `wadiz.io` 로 이전 (8커밋)
> - stage 환경 도메인·E2E 주소·studio stage 환경 파일·코드 내 stage 참조를 **전부 `wadiz.io`** 로 바꿨습니다.
> - 빌드 스크립트에 **stage 분기**를 넣고 **static stage 빌드 기준을 `clive` 로** 바꿨습니다. 계정 origin 매핑 2곳에도 stage 분기를 추가했습니다.
> - WAi 런처를 **`.env-cmdrc` 방식으로 전환**하고 `environment` 옵션을 제거했습니다.
> - 백엔드 짝: [`com.wadiz.web`](../com.wadiz.web.md) RWD-6044, 인프라는 [`helm-charts`](../helm-charts.md) 의 `web/stage`·`user-platform/stage` 신설입니다.
>
> ### FE2-1255 — 새소식 예약 공개 UI (20커밋)
> - 스튜디오에 **새소식 예약 공개** 화면을 만들었습니다 — 예약 상태 추가, 예약 일시 선택, **선택 가능 기한 제한**, 안내 모달.
> - **발행이 끝났거나 예약 시각이 지난 새소식은 예약 설정을 잠급니다.** 예약 시각이 지난 응답을 받으면 안내 모달을 띄웁니다.
> - 고친 것들: 발행 완료 새소식 수정 시 **예약 정보가 사라지던 문제**, 예약 게시 선택 시 **예약 일시 기본값 누락**, 임시저장 건을 예약 저장하면 **발행 완료 모달이 뜨던 문제**, 가이드 모달 반복 노출, 데스크톱에서 예약 영역으로 스크롤되지 않던 문제.
> - 알림 안내 문구에서 **"즉시 발송" 표현을 제거**했습니다(예약이 생겨 더 이상 사실이 아님).
> - 백엔드 짝: [`com.wadiz.api.funding`](../com.wadiz.api.funding/com.wadiz.api.funding.md) 의 RWD-6036·6037·6038·6041 입니다.
>
> ### CLIENT-255 / CLIENT-256 — Sentry 정비
> - **봇 판정을 Sentry `beforeSend` 에 연결**하고 `bot_reasons` 태그를 붙입니다. 판정 함수는 `packages/core/src/metrics/botDetection.ts` 로 옮겼습니다. 판정은 **첫 이벤트로 미루고 태그만** 답니다.
> - ⚠️ **clive 에서 실사용자 이메일이 Sentry `userInfo` 에 실리던 문제를 고쳤습니다.**
> - global·static 번들 Sentry 에 **`allowUrls`** 를 넣어 `wadiz.io` 지면의 에러를 수집하도록 했습니다(FE2-1295 에서 허용 URL 도메인도 `wadiz.io` 로 변경).
>
> ### CLIENT-260 — 정기 배포 워크플로
> - `event-regular-deployment-apps-ci-cd` 워크플로를 추가하고, 예약 워크플로가 `app-regular-deployment-ci-cd` 를 실행하도록 했습니다. stage CI/CD 실행은 예약 워크플로에 **인라인으로 합쳤습니다.**
>
> ### A2UI 후속 (CLIENT-251 · 254 · 247)
> - **`A2uiStackedBarChart`** 를 추가해 카탈로그 v1 에 등록했습니다(직전 회차에 WDS 쪽 `StackedBarChart` 를 만든 것의 A2UI 래핑). 생성형 UI 수집 화면 이름에도 스택막대차트를 넣었습니다.
> - **범례 항목을 클릭해 활성 항목을 선택**할 수 있게 했고(CLIENT-254), 범례 버튼의 줄 바꿈 이름이 가운데 정렬되던 문제를 고쳤습니다.
>
> ### 기타
>
> | 이슈 | 내용 |
> |---|---|
> | FE-8218 | **통합기획전 BFF 연동** — 기획전 배치 로직을 분리하고 [`app-api`](../app-api.md) 의 BFF 를 호출합니다. **서버 캐시를 국내·해외로 나눠 on/off** 할 수 있고, BFF 가 400 을 주면 폴백하지 않고 warning 으로 기록합니다 |
> | FE1-1780 | **카테고리 페이지를 `apps/global` 로 이전**([`com.wadiz.web`](../com.wadiz.web.md) 의 global 쉘 전환과 짝). 데스크톱 sticky 위치 보정, 스타일을 공용 유틸리티 클래스로 대체 |
> | FE1-1867 | Braze 미초기화 상태에서 **호출하지 않도록 게이트** 추가, 전송 호출을 예외로부터 격리 |
> | FE2-1209 | 영상 인앱 메시지에 **세로형 4:5 타입** 추가 및 모달 스크롤 처리 |
> | FE1-1871 | 메일 템플릿 아이콘 이미지 도메인을 **`cdn-static.wadiz.io`** 로 교체 |
> | QA-23389 | 서포터클럽 미가입 시 소개 페이지로 **풀 페이지 이동** |

> 📅 **2026-09-10 cloud_live pull 보강** (250 커밋, 484파일 +15,039/−7,289)
>
> **생성형 UI(A2UI) 카탈로그 구축이 절반 이상**이고, **멤버십 결제 유예(GRACE_PERIOD) 지면 대응**이 그 뒤를 잇습니다. i18n 자동 동기화 커밋은 생략합니다.
>
> ---
>
> ## 1. A2UI — WAi 어시스턴트가 그리는 화면의 부품 카탈로그 (약 110커밋)
>
> `CLIENT-210 · 211 · 215 · 218 · 220 · 221 · 223 · 226 · 231 · 246 · 247 · 248 · 250`
>
> **A2UI 가 무엇인가**: AI 어시스턴트(WAi)가 답변을 텍스트가 아니라 **화면 부품(카드·차트·선택지)으로 내려보내면 앱이 그것을 그리는** 구조입니다. 서버가 "어떤 부품을 어떤 값으로 그려라" 를 JSON 으로 보내고, 프론트가 카탈로그에 등록된 컴포넌트로 렌더합니다.
>
> **위치**: `apps/global/src/features/wai/ui/A2ui/` — 버전 폴더 `versions/v0_9/` 안에 `A2uiProvider` · `A2uiRenderer` · `components/` 가 있고, `catalogs/v1/catalog.json` 이 서버와 주고받는 계약입니다. 카탈로그는 `catalogs/extract-catalogs.ts` 로 **컴포넌트 스키마에서 자동 추출**합니다.
>
> ### 블록(Block) — 답변 한 덩어리 단위
>
> | 블록 | 용도 |
> |---|---|
> | `AdRecommendationBlock` | 광고 추천 카드(모바일은 캐러셀) |
> | `QuestionSingleAnswerBlock` · `QuestionMultipleAnswerBlock` | 단일/다중 선택 질문 |
> | `SuggestedQuestionBlock` | 추천 질문 칩 |
> | `NextActionBlock` | 다음 행동 유도 — 칩 목록 구조로 바꿨다가 **제거됐습니다** |
>
> ### 기본 컴포넌트
> - `A2uiColumn` · `A2uiRow` · `A2uiImage`(비율 3:2 통일) · `A2uiLink` · `A2uiBadge` · `A2uiCheckbox` · `A2uiListItem`(선택 행) · `A2uiResponsiveList`(반응형 목록).
> - 마크다운 링크는 `Text` 에서 `openUrl` 로 처리하고 별도 `Link` 컴포넌트는 제거했습니다.
>
> ### 차트 4종 — `packages/waffle` 에 WDS 컴포넌트로 신설
> - **`BarChart` · `GroupedBarChart` · `StackedBarChart` · `DonutChart`**. 렌더링은 **visx**(`@visx/group`·`@visx/legend`)를 씁니다.
> - 공용 타입·함수(`Chart.types.ts`·`Chart.constants.ts`·`lib/`), 범례 `OrdinalLegend`(배치 방향·값 표시·배지 지원), 축 배지 `Badge`·`TickBadgeGroup`, 툴팁이 함께 들어왔습니다.
> - A2UI 쪽은 자체 구현을 쓰다가 **전부 WDS 차트로 교체**했습니다(`A2uiBarChart` → WDS `BarChart`, `A2uiPieChart` → `A2uiDonutChart`).
> - 배지 폭은 **10px 글자 기준으로 계산**하고 y축 배지는 축 선 4px 옆에 둡니다. 글자 폭·줄 나누기 함수는 `lib/text.ts` 로 합쳤습니다.
>
> ### 대기 상태(Skeleton)
> - `packages/waffle` 에 **`Skeleton` 컴포넌트**(Box·Text·컨테이너 gap prop)를 신설하고, A2UI 의 자체 스켈레톤을 이것으로 교체했습니다. 차트마다 전용 스켈레톤(`GroupedBarChartSkeleton`·`DonutChartSkeleton`)도 있습니다.
>
> ### 데이터 수집 (CLIENT-226)
> - **생성형 UI 의 노출·클릭을 수집**합니다. 노출 수집에 **서피스 식별자**(어느 지면에서 떴는지), 다중선택 제출에 **선택한 답변 문구**를 담습니다. 클릭 수집은 액션 처리기와 `openUrl` 구현으로 옮겼습니다.
>
> ### 문서·규약
> - `A2ui/CLAUDE.md` · `NAMING_CONVENTIONS.md` · `catalogs/update-component-to-wiki.prompt.md` — **카탈로그를 위키에 반영하는 프롬프트까지 저장소에 두었습니다.** 배지 문구는 명사구 15자 이내, 목(mock) 문구 정형화, 빈 값 안내 규칙 등이 지침으로 고정돼 있습니다.
>
> ---
>
> ## 2. 멤버십 결제 유예(GRACE_PERIOD) 지면 대응 (약 45커밋)
>
> `FE1-1619 · 1631 · 1632 · 1633 · 1634 · 1635 · 1636 · 1637 · 1671`
>
> 백엔드가 8개 저장소에 `GRACE_PERIOD` 상태를 넣은 것([`com.wadiz.api.funding`](../com.wadiz.api.funding/com.wadiz.api.funding.md) RWD-5899 등)에 맞춰, 지면 전반을 손봤습니다. 서포터클럽 약관 개정(FE1-1761, **결제 실패 후 7일 유예**)의 화면 구현입니다.
>
> ### 판정 축을 둘로 나눴습니다 (FE1-1631 · 1634 · 1635)
> - 기존에는 `getSubscribed` 같은 **구독 여부 하나**로 모든 걸 판단했습니다. 이제 **`hasMembership`(가입했는가)** 과 **`canUseBenefit`(지금 혜택을 쓸 수 있는가)** 두 축입니다. 결제 유예 중이면 앞은 참, 뒤는 거짓입니다.
> - 지면별로 어느 축을 쓸지 정리했습니다 — **쿠폰 다운로드 배너·만족도 뱃지는 가입 축**, **펀딩 결제 특별가·배송비 할인·스토어 상세 금액은 혜택 축**. 소비처가 없어진 `getSubscribed` 정의는 삭제했습니다.
> - 목(mock) 핸들러에 **결제유예 회원을 재현하는 분기**를 넣어 개발 중 확인이 가능하게 했습니다.
>
> ### 재결제 유도 동선 (FE1-1633 · 1671)
> - 결제 금액 카드 · 스토어 결제 배너 · 카드 등록 모달 · 결제 지면 유예 배너에 **재결제(멤버십 재개) 유도**를 붙였습니다.
> - 이후 **호출처 6곳을 "멤버십 가입 모달" 하나로 통합**했습니다(FE1-1671) — 같은 로직이 여기저기 흩어지는 것을 막은 정리입니다. `CardRegistrationModal` 의 재개 로직은 제거했습니다.
>
> ### 유예 중 막는 것들 (FE1-1619)
> - 결제유예 상태에서는 **상품 변경 대신 재결제 절차로 진입**하고, 해지 시 상품 변경 유도 모달을 생략하며, **쿠폰존의 서포터클럽 배너를 띄우지 않습니다.**
> - 결제 실패 안내 문구에서 **재시도 시점을 단정하는 표현을 뺐습니다**(정확한 재시도일을 보장할 수 없기 때문). 결제내역 모달 타이틀에서도 다음 결제일을 제거했습니다.
> - 클릭 수집 라벨에 **유예 접미사**를 붙여 유예 상태의 행동을 따로 볼 수 있게 했고, 이 라벨 규칙을 공용화했습니다.
> - **멤버십 상태 모델 정책 문서**와 스토어 상세 혜택 배너 유예 정책이 저장소 문서로 들어왔습니다.
>
> ### 안내·관리 화면 (FE1-1632 · 1636 · 1637)
> - 서포터클럽 관리 페이지에 결제유예 안내를 띄우고, 결제 카드에 **결제유예 실패 타입**을 추가했습니다.
> - 어드민 회원 검색 필터에 **결제유예 상태 옵션**을, 유예 회원에게 **강제 해지 링크**를 노출합니다.
> - 마이와디즈 서포터 탭 띠배너에 **`graceUntil`(유예 종료일)을 배선**했습니다. 이를 위해 **마이와디즈 카드 API 를 v6 로 올렸습니다**([`main2-api`](../main2-api.md) DISPLAY-1709 와 짝).
>
> ---
>
> ## 3. WAi 메이커 스킬 (FE2-1129 · FE2-1267)
> - WAi 입력창에 **스킬 셀렉터**를 넣고 **광고 추천 스킬**을 도입했습니다. 로그인 유도 띠를 **입력 카드 헤더로 통합**하고, 카드형 변형과 드롭 오버레이를 내재화했습니다(`packages/waffle` 의 `WAi/PromptInput`).
> - 스킬 식별자를 `feature` 로 통일하고, WebSocket `user_message` 에 `features` 를 **항상 포함**하도록 했습니다(필수 스펙). `userName` 파라미터는 **live·clive 환경에서 제외**합니다.
> - FE2-1267: **광고 추천 워크스루 모달**을 추가하고 **국내 전용으로 노출**합니다. 닫기 이벤트를 수집하며, 체류 시간 수집은 넣었다가 제거했습니다.
>
> ## 4. 글로벌 SPA 이관 (FE1-1751 · FE1-1754 · FE1-1756)
> - **서포터클럽 소개 · 앱 설치 랜딩 · 서비스 제공 현황** 세 지면을 `apps/global` 의 한국 라우트로 이전했습니다([`com.wadiz.web`](../com.wadiz.web.md) 쪽 뷰 전환과 짝).
> - 이관 후 **static 화면과 픽셀 단위로 맞추는 수정**이 이어졌습니다(CTA 여백을 안쪽 요소로 옮겨 높이에 포함, 모바일 헤더 복원, 표 높이 복원 등).
> - 리다이렉트로 도달 불가해진 약관 페이지 3종과 미사용 landing 엔트리를 제거했습니다.
>
> ## 5. 죽은 코드 정리
> - **FE1-1829** — static 더보기 앱(`main-more-app`) 제거(직전 회차 FE1-1778 이관의 마무리).
> - **FE1-1782** — 와디즈 넥스트(`/web/wadiznext`) 죽은 코드 삭제.
> - **FE1-1826** — **찜 마감임박 조회를 검색 서버에서 `wish-api` 로 전환**하고 검색 서버 호출 잔재를 제거했습니다. 위시 뱃지 저장 키를 호출 시점에 계산하도록 고치고, 조회 실패를 기록하도록 했습니다.
>
> ## 6. 기타
>
> | 이슈 | 내용 |
> |---|---|
> | FE1-1818 | 해외 참여 펀딩 안내 팝업 문구 수정 + 다국어 키 추가 |
> | FE2-1272 | 대시보드 카드 조회의 **timezone 쿼리 파라미터 URL 인코딩 누락** 수정 |
> | QA-23220 | 멤버십 결제 안내 카드 너비를 영역에 맞춤 |
>
> ---

> 📅 **2026-09-08 cloud_live pull 보강** (61 커밋)
>
> **이미지 로딩 통합(FE1-1766·1735·1736·1763·1765)** 이 최대 테마이고, **더보기 페이지의 글로벌 SPA 이관(FE1-1778)** 과 잔버그 수정이 뒤를 잇습니다. i18n 자동 동기화 커밋 다수는 생략합니다.
>
> ### 이미지 로딩 정본화 — 포크 4벌을 하나로 (FE1-1766 · FE1-1763)
> - 같은 일을 하는 이미지 컴포넌트가 여러 벌 복제돼 있었습니다. **`OptimizedImage` 하나로 통합**하고(`FE1-1766`, 2단계), 이미지 주소 계산을 **`getOptimizedImageURL`** 로 분리했으며 지연 로딩 방식을 옵션으로 뺐습니다.
> - `ThumbnailImage` **사본 3벌 + 홈 키비주얼 사본 1벌**도 `packages/ui` 의 정본으로 흡수했습니다(FE1-1763).
> - PR 리뷰 반영으로 `optimized-image` 를 `packages/ui` 로 옮기고 우선 로딩 판정을 보정했습니다. 이미지 품질값은 **80** 으로 조정.
> - 부수 수정: static 빌드에서 scss 변수를 못 찾던 문제, 배럴(index) 이 없는 수출을 가리키던 문제, 개발 모드 이미지 뷰어 문제.
>
> ### 이미지 포맷·CDN (FE1-1736 · FE1-1734 · FE1-1735 · FE1-1733)
> - **이미지 기본 포맷을 webp 로 승격**했습니다(FE1-1736). 포맷 허용 목록 정규식의 **앵커를 교정하고 webp 를 허용**(FE1-1734) — 앵커가 없어 의도치 않은 문자열이 통과하던 문제로 보입니다.
> - 프로젝트 카드 엠블럼에도 이미지 최적화를 적용(FE1-1735).
> - FE1-1733(클라우드 CDN 호스트 인식 확장·이중 변환 방지)이 이번 범위에 다시 들어왔습니다 — [`app-api`](../app-api.md) CLIENT-235 와 같은 주제입니다.
> - **슬로건 로고를 GIF → mp4 로 교체**해 성능을 개선했고(FE1-1765), 리렌더 때 재생이 반복되던 문제를 ref 고정으로 막았습니다.
> - 서비스홈 키비주얼 배너는 **첫 화면 이미지를 먼저 받도록** 바꿨습니다(FE1-1764).
>
> ### FE1-1778 — 더보기 페이지를 글로벌 SPA 로 이관
> - 더보기 메뉴 기능을 **`packages/features/more-menu` 로 이식**하고 더보기 페이지·한국 라우트를 추가했습니다. [`com.wadiz.web`](../com.wadiz.web.md) 쪽에서 `/web/main/more` 를 `global-korea/index` 로 전환한 것과 짝입니다.
> - 모바일 레이아웃에서 **라이브에 없는 푸터가 뜨던 문제**를 제거했습니다.
>
> ### 공유 기능 수정 (FE1-1808 · FE1-1813)
> - **아이폰 웹에서 페이스북 공유 시 앱만 열리고 끝나던 문제**를 고쳤습니다. 공유 주소의 프래그먼트를 URL 객체로 설정하도록 바꿨습니다(FE1-1808).
> - 랜딩·기획 지면의 공유 버튼이 아예 동작하지 않던 문제와 트렌드리포트 공유 버튼 아이콘 크기를 수정했습니다(FE1-1813).
>
> ### 잔버그 수정
>
> | 이슈 | 내용 |
> |---|---|
> | FE1-1809 | 지지서명 **생성 전 서명 여부 확인이 누락**돼 있던 것을 수정하고, 잠금 기준을 일원화·조회 로직을 공통화 |
> | FE1-1807 | 1:1 문의 사진 전송의 **포맷 검증 정규식과 에러 안내 누락** 수정, `image/jpg` MIME 허용 추가 |
> | FE1-1803 | 메시지함 — 프로젝트 필터 화살표 정렬·폭 초과 수정, 문구를 **탭 기준으로 분기**하고 서포터 문구를 "메이커 문의 안내" 로 변경(모바일 헤더 제목 포함) |
> | FE1-1801 | 위시 카드에 **쿠폰 배지** 노출 추가, 배지 굵기 bold, 텍스트 영역 높이를 내용에 맞춰 이미지와 가운데 정렬 |
> | FE1-1814 | 에디션 지면 제목에 다국어 키 적용, 에디션·친구 지면 모바일 헤더와 하단 네비게이션 정비 |
>
> ### 기타
> - **FE2-1209** — 브레이즈(Braze) **영상 인앱 메시지 템플릿** 추가(세로형 너비 수정 포함).
> - **FE2-1176** — **AI 심사 프로젝트 비노출 안내 메일** 템플릿 추가(v2 문구·버튼 구성).
> - **FE2-1232** — 스튜디오 E2E 대상 환경을 **클라우드 환경으로 정리**했습니다(`studio/{funding,maker,store}-e2e` 의 `.env.example`·`helpers/env.ts`·`playwright.config.ts` 일괄).
> - **FE1-1799** — eslint 계열 패키지 버전 혼재를 정리하고 루트 호이스트 예외를 제거했습니다. 그에 따라 불필요해진 static preinstall 의 환경 변수 차단도 삭제.
> - **FE1-1672** — 막펀잡기 **웹 안내 페이지** 신설(QR 섹션이 렌더되지 않던 `qrcode.react` 1.x 해석 문제 수정 포함).
>
> ---
