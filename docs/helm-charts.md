...
>
> 2026-09-03 보강에 "rc1 환경이 전 플랫폼에서 삭제됐다" 고 적었습니다. 그 뒤 두 번 더 바뀌었습니다.
>
> | 시점 | 무슨 일이 있었나 |
> |---|---|
> | 2026-09-03 | `values/{플랫폼}/rc1/` 디렉터리 57개 파일이 삭제됐습니다. 최상위 `rc1.yaml` 만 고아로 남았습니다 |
> | 2026-09-10 | 커밋 `cb127a1`(SRECLOUD-700)이 **고아로 남아 있던 `live.yaml`·`rc1.yaml`·`rc2.yaml` 세 개를 삭제**했습니다 |
> | **2026-09-16** | 커밋 `8667d88` 이 **`rc1.yaml` 과 `client/rc1/app-api.yaml` 을 새로 만들었습니다** |
>
> 되살아난 rc1 은 **배포 파이프라인을 시험하려고 만든 것**입니다. 커밋 제목이 "rc1 app-api 테스트 배포용 값 파일 추가" 입니다.
> 지금 rc1 에 있는 서비스는 `app-api` 하나뿐이고, 플랫폼도 `client` 하나입니다.
>
> 새 `rc1.yaml` 의 내용은 이렇습니다.
> 게이트웨이 호스트는 `api.rc1.wadiz.io`, Datadog 환경 이름은 `rc1`, 활성 프로파일은 `rc1` 입니다.
> CORS 허용 주소는 `rc1.wadiz.io`·`adm.rc1.wadiz.io`·`account.rc1.wadiz.io` 세 개입니다.
>
> `client/rc1/app-api.yaml` 은 사내 대역(`192.168.200.0/24`)만 허용하고, 메모리는 0.2Gi 입니다.
> [`helm-charts-gitops`](./helm-charts-gitops.md) 에도 같은 파일이 함께 생겼습니다. 다만 **토큰은 넣지 않았다**고 커밋에 적혀 있습니다.
>
> ### rc4 친구톡 API 증설
>
> `display-platform/rc4/friendtalk-api.yaml` 의 복제 수와 메모리를 늘렸습니다.
> 같은 기간에 `kr.wadiz.platform.api.friendtalk` 저장소에서 **카탈로그형 브랜드 메시지**(DISPLAY-1726) 작업이 진행 중입니다.
>
> ### 통계
>
> 서비스 값 파일 **308 → 309개**, 고유 서비스 이름 **110개 유지**입니다.
> 환경 디렉터리는 `clive`·`dev`·`rc4` 에 더해 `stage`(display-platform·user-platform·web)와 **`rc1`(client 한 곳)** 이 있습니다.
...
>   - 📅 **2026-09-10 정정**: 이 고아 파일 3개는 커밋 `cb127a1`(SRECLOUD-700)로 **모두 삭제**됐습니다. 이후 `rc1.yaml` 만 2026-09-16 에 시험 배포용으로 다시 생겼습니다.
