...
> | 이슈 | 내용 |
> |---|---|
> | **FE1-1957** | **배포 환경을 클라우드 네 갈래로 정리**하고 자동 배포를 연결했습니다. 이미지 빌드 워크플로에서 55줄이 빠지고, 매일 브랜치를 합치는 워크플로에 88줄이 늘었습니다 |
> | **FE1-1960** | 데이터베이스 계정을 고를 때 쓰던 **클라우드 분기를 제거**했습니다(`src/config/typeorm.config.ts`). 온프레미스가 없어져 분기가 의미를 잃었습니다 |
>
> `src/common/constants/environment.constants.ts` 도 함께 정리됐습니다.
>
> ---

> 📅 **2026-09-17 cloud_live pull 보강** (1 커밋)
>
> ### CLIENT-271 — `robots.txt` 를 단일 정책으로 통일
>
> `robots.txt` 는 검색엔진 크롤러에게 어디를 읽어도 되는지 알려 주는 파일입니다.
> 종전에는 특정 봇 이름을 따로 적어 그룹별로 규칙을 줬습니다. 그 명시 그룹 10줄을 지우고 **모든 봇에 같은 규칙**을 적용합니다.
>
> 직전 회차의 CLIENT-270(`/links/`·`/redirect/` 허용 추가)에 이은 정리입니다.
>
> ---
