...
> 📅 **2026-09-22 master pull 보강** (21 커밋)
>
> **PD 컨설팅 신청형(V3) 개편(RWD-5954, 17커밋)** 이 대부분입니다.
> PD 는 프로젝트를 봐 주는 와디즈 담당자입니다. 메이커가 컨설팅을 신청하면 PD 가 심사합니다.
>
> ### RWD-5954 — 신청형 컨설팅과 PD 스크리닝
>
> **신청 방식이 바뀌었습니다.** 기존에는 자동으로 붙던 것을 메이커가 신청하는 형태로 바꿨습니다.
>
> | 변경 | 내용 |
> |---|---|
> | V3 신설 | PD 컨설팅 신청형 버전을 새로 만들고 **기본값을 V3** 로 뒀습니다. 제출 후에는 저장을 막습니다 |
> | 수동 스크리닝 복원 | V3 를 한 번 되돌렸다가 **수동 스크리닝을 살려** 다시 넣었습니다. 부가서비스 상태를 원자적으로 맞춥니다 |
> | 게이트 이동 | PD 수동 스크리닝 판정 기준을 **심사 단계에서 프로젝트 상태로** 옮겼습니다 |
> | 자동 승인 기준 | 어드민이 신청으로 전환할 때의 자동 승인 판정을 **오픈 여부에서 최종 승인 여부로** 바꿨습니다 |
> | 반려 처리 | V3 반려를 `PD_REJECT` 로 바꾸고 **반려 알림(메일 1489·문자)** 을 연결했습니다 |
>
> **서버 검증을 세 곳에 넣었습니다.** 화면에서만 막던 것을 서버에서도 막습니다.
>
> - 신청 전환 시 PD 담당자가 지정됐는지 확인합니다.
> - 스크리닝 승인 시에도 PD 담당자를 확인합니다.
> - **종료된 프로젝트는 컨설팅 상태를 바꿀 수 없습니다.**
>
> **반려 모달에서 고친 결함**
>
> | 결함 | 고친 방법 |
> |---|---|
> | 이전 회차에 해제한 체크박스 상태가 다음 회차까지 남던 문제 | 초기화를 `attr` 에서 `prop` 으로 바꿨습니다 |
> | 알림 채널을 고르지 않아도 반려가 되던 문제 | 미선택 시 알림창으로 막습니다 |
> | 발신자 표기가 실제와 달랐던 문제 | 실제 발신 주소 `info@wadiz.io` 로 고정했습니다(QA-23426) |
> | 반려 상태가 필터에 두 번 뜨던 문제 | 폐기된 상태를 제외했습니다. 반려 검색은 V1 잔존 데이터를 포함합니다 |
>
> ### 보안 수정 두 건
>
> | 이슈 | 내용 |
> |---|---|
> | **RWD-6087** | **API 프록시에서 `internal` 경로 전달을 차단**했습니다. [`com.wadiz.web`](./com.wadiz.web.md) 에도 같은 수정이 있습니다 |
> | **RWD-6090** | 리워드 프로젝트 목록과 상세 팝업의 **프로젝트명을 HTML escape** 처리했습니다. 프로젝트 이름에 태그를 넣어 화면을 조작하는 것을 막습니다 |

> 📅 **2026-09-17 master pull 보강** (22 커밋)
>
> **인공지능 심사 화면 개편(RWD-5975, 16커밋)** 이 대부분입니다.
> 인공지능이 프로젝트 스토리를 검토하는 기능이고, 어드민에서 그 결과와 이력을 봅니다.
>
> ### RWD-5975 — 인공지능 심사를 "심사 주체" 축으로 다시 짰습니다
>
> 가장 큰 변화는 **자동 심사와 요청 심사를 구분해서 보게 된 것**입니다.
>
> - 어드민의 인공지능 심사 상태 조회를 **심사 주체별(자동·요청)** 로 개편했습니다.
> - 리워드 프로젝트 목록에 **심사 주체 필터와 심사 결과 필터**를 추가했습니다.
> - 신규 모델 `AIReviewStatusRow`(26줄)·`AIReviewSubjectStatus`(30줄)가 들어왔고, `AIReviewManagement` 는 `AIReviewUsageCount` 로 이름이 바뀌었습니다.
>
> **사후심사 이력 화면을 손봤습니다**
>
> | 변경 | 내용 |
> |---|---|
> | 인공지능 심사 이력 표시 | 사후심사 이력에 인공지능 심사 행을 함께 보여 줍니다. 심사 탭 화면도 함께 개편했습니다 |
> | 언어 기록 누락 정정 | 사후심사 이력을 저장할 때 `LanguageCode` 가 빠지던 것을 고쳤습니다 |
> | 기본 정렬 | 내림차순으로 바꿔 최신 이력이 위에 오게 했습니다 |
> | 결과 색 표시 | 심사 결과별로 글자색을 달리 합니다 |
> | 비고 표기 | 언어를 한글 이름으로 적고, 제출 불가 행에는 심사 결과 페이지 링크를 붙였습니다 |
>
> **고친 결함 세 가지**
>
> - 목록의 비즈모델 필터에서 **열거형과 문자열을 비교하다 나던 OGNL 오류**를 고쳤습니다.
> - 인공지능 심사 주체 필터가 **마지막 심사의 주체로 판정되지 않던 문제**를 고쳤습니다(QA-23392).
> - 인공지능 심사 필터를 **언어별 최신 심사 기준**으로 정정했습니다.
>
> 스튜디오로 넘어가는 딥링크의 URL 파라미터는 케밥 케이스(`kebab-case`, 단어를 붙임표로 잇는 표기)로 바꿨습니다.
>
> ### FE1-1859 — 앱 설정 관리 화면이 어드민에서 빠졌습니다
>
> 앱 설정 관리 화면을 어드민에 붙였다가, **중계를 걷어내고 화면이 `app-api` 를 직접 부르도록** 바꿨습니다.
> 어드민은 이제 진입점(사이드바 링크)만 갖습니다. 실제 화면은 [`wadiz-frontend`](./wadiz-frontend/wadiz-frontend.md) 의 FE1-1854·FE1-1858 에 있습니다.
> 메뉴 경로는 `console` 하위로 옮겼다가 편성 화면만 `campaigns` 로 다시 옮겼습니다.
>
> ### RWD-6064 — PUT 요청 본문이 사라지던 문제
>
> `apip` 프록시 경로에서 **`FormContentFilter` 를 제외**했습니다.
> 이 필터가 요청 본문을 미리 읽어 버려서, 프록시로 넘길 때 본문이 비는 현상이 있었습니다.
