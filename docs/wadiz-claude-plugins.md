...
>
> ### `weekly-report` 스킬에 실행 스크립트 추가
>
> 주간 보고서를 만드는 스킬에 **파이썬 스크립트 4개**가 들어왔습니다.
> 파라미터로 받아 다시 쓸 수 있는 형태입니다.
>
> | 파일 | 역할 |
> |---|---|
> | `scripts/render.py`(373줄) | 보고서 그리기 |
> | `scripts/model.py`(228줄) | 데이터 모델 |
> | `scripts/publish.py`(50줄) | 게시 |
> | `scripts/window.py`(31줄) | 기간 계산 |
>
> 판정 규칙을 개정하고 수집 결함 3건을 고쳤습니다.
>
> ### 매니페스트 오류 수정
>
> `plugin.json` 의 오류를 고쳤습니다(`shared` 0.7.3, `fe2` 0.9.1).
> `merge-branch` 스킬의 문서 톤도 정리했습니다(300줄 수정).
...
>
> ### `merge-branch` 스킬 보완
> - 직전 회차에 추가된 `merge-branch` 스킬(`plugins/shared`)을 보완했습니다(+145/−16) — **fetch 검증**, **변경 규모 계산**, **PR 머지 가능 여부 확인**이 들어갔습니다. 플러그인 버전도 함께 올렸습니다.
>
> ---

> 📅 **2026-09-08 main pull 보강** (4 커밋)
>
> 스킬 2종이 새로 추가됐습니다.
>
> ### `weekly-report` — 클라이언트개발팀 주간 리포트 자동 생성 (`plugins/client`)
> - Jira 이니셔티브·에픽 데이터로 팀 주간 리포트를 만들어 Confluence 주간 회의 페이지로 올리는 스킬입니다(`SKILL.md` 259줄).
> - 후속 커밋에서 위키 페이지 **상단에 주간 회의 아티팩트 링크 섹션**을 넣도록 보강했습니다.
> - plugin.json 버전 갱신은 없었습니다(`plugins/client` 만 1줄 변경).
>
> ### `merge-branch` — 브랜치 병합 PR 생성 (`plugins/shared`)
> - 브랜치를 병합하는 PR 을 만들어 주는 스킬입니다(`SKILL.md` 365줄). `plugins/shared` 플러그인 버전을 **0.6.2 → 0.7.0** 으로 올렸습니다.
>
> ---
