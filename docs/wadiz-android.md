...
> 이미지 성능 개선도 다섯 이슈에 걸쳐 있습니다.
>
> ### FE1-1760 — `kr` 도메인 코드를 앱에서 걷어냈습니다 (13커밋)
>
> 온프레미스 시절의 `wadiz.kr` 서버를 가리키던 코드를 통째로 지웠습니다.
>
> | 지운 것 | 내용 |
> |---|---|
> | 서버 환경 정의 | `kr` 서버 환경 자체를 없앴습니다 |
> | 판정 분기 | 서비스 인터페이스와 `isCloud` 분기를 **하나로 합쳤습니다** |
> | 전환 게이트 | 전환 판정과 부팅 대기 게이트를 제거했습니다 |
> | R8 규칙 | 죽은 R8 keep 규칙을 지웠습니다 |
> | 링크 | 하드코딩된 바깥 링크를 정리했습니다 |
> | Stage 분기 | Stage 에 기대던 분기를 제거했습니다 |
>
> 기획 문서와 검증 문서(PRD·TC)를 함께 썼고, 클라우드 분기용 도메인 추상화도 정리했습니다.
> 웹의 `FE1-1944`(잔여 `wadiz.kr` 치환)와 같은 흐름입니다.
>
> ### FE1-1850 — 막펀잡기 전환율 개선 (19커밋)
>
> 막펀잡기는 마감이 임박한 프로젝트를 카드로 넘겨 보는 화면입니다.
> 전환율을 올리려고 화면과 조작 방식을 손봤습니다. iOS 의 `FE1-1848`(32커밋)과 짝입니다.
>
> 관련 수정이 셋 더 있습니다.
>
> - **FE1-1864** — 좁은 세로 화면에서 미리보기가 뜨지 않던 문제를 고쳤습니다. 판정을 비율 규칙에서 **카드가 화면을 얼마나 덮는지** 기준으로 바꿨습니다.
> - **FE1-1930** — 막펀잡기 카드의 노출 수집 조건을 고쳤습니다.
> - **FE1-1625** — 서포터클럽 결제 실패 유예 기간 확대를 반영했습니다.
>
> ### 이미지 성능 (FE1-1881 · 1885 · 1886 · 1890 · 1900)
>
> | 이슈 | 내용 |
> |---|---|
> | FE1-1885 | **CDN 이미지 기본 포맷을 WebP 로** 전환했습니다 |
> | FE1-1886 | WebP 변환 시 **`autorotate` 옵션**을 붙였습니다. 사진 방향 정보가 사라지는 것을 막습니다 |
> | FE1-1900 | CDN 크기 조정 요청을 **폭 기준 100 단위 버킷**으로 바꿨습니다. 같은 이미지를 조금씩 다른 크기로 여러 벌 만들지 않게 됩니다 |
> | FE1-1890 | CDN 주소 유틸을 `core-common` 으로 옮겼습니다 |
> | FE1-1881 | 이미지 크기 관련 성능 개선. 검증 문서의 실측값을 갱신하고 독립 리뷰를 받았습니다 |
>
> ### 그 밖에
>
> | 이슈 | 내용 |
> |---|---|
> | FE1-1911 | **Stage 환경 지원**을 넣었습니다 |
> | FE1-1693 | 영상 디코더를 과하게 점유하던 문제를 고쳤습니다 |
> | FE1-1880 | 검색홈 피드 영상의 `PreloadManager` 를 제거했습니다 |
> | FE1-1879 | 소셜 회원가입 중복 요청으로 500 이 나던 문제를 고쳤습니다 |
> | FE1-1869 | 알림함 아이콘 대체 경로의 `cloudfront.net` 직접 참조를 없앴습니다 |
> | FE1-1904 | 스토어홈 큐레이션의 수집 태그에서 상품 타입 접미사를 뺐습니다 |
> | FE1-1416 | 앱 설정 폴백 동기화 액션을 추가했습니다 |
> | FE1-1797 | 카카오 SDK 버전을 올렸습니다 |
> | SDQA-11 · SDQA2 | 위로 스와이프 아이콘과 토스트 뱃지 디자인 수정 |

> 📅 **2026-09-10 main pull 보강** (142 커밋, v26.36.0 build 594 → **v26.37.0 build 595**)
>
> **막펀잡기 정보성 알림(FE1-1641·1648·1649·1823, 40커밋)** 이 최대 테마이고, **출시율 자동화(FE1-1562)** 와 **와디태그 수집 오류 수정(FE1-1370)** 이 뒤를 잇습니다. (버전 자동 업데이트·다국어 리소스 동기화 커밋 다수 생략)
>
> ### 막펀잡기 정보성 알림 (FE1-1648 · 1649 · 1641 · 1823)
> - 막펀잡기(따라잡기) 프로젝트의 마감을 알려 주는 **정보성 알림**을 붙였습니다. 알림 **유도 모달 3종**과 수신 연동 모달 노출 로직, 설정 화면을 만들었습니다.
> - **A/B 테스트와 다국어·데이터 수집**이 함께 들어갔습니다(FE1-1649).
> - 서버 API 스펙 변경 반영과 디자인 QA 가 이어졌습니다. 수신동의 API 는 [`kr.wadiz.platform.crm`](./display-platform-services-3-messaging.md) 의 DISPLAY-1702 이고, iOS 의 짝은 **FE1-1638**(25커밋)입니다.
>
> ### FE1-1562 — 출시율(rollout) 자동화 (14커밋)
> - 직전 회차의 릴리즈 게시 자동화(FE1-1561)에 이어 **출시율을 단계적으로 올리는 스킬**을 만들었습니다(`release-rollout`).
> - **게이트 기준을 "PV" 에서 "버전별 기기당 PV" 로 교체**하고, 사용 지표 임계값을 **0.9 로 확정**했습니다. 다음 단계 출시율은 사다리(ladder)에서 **min 으로 선택**합니다(리뷰 반영).
> - **전체 출시 후 실사용 비율을 5일 추적**하고, 사용 현황을 조회형 리포트로 뽑습니다(출처 표기 포함). 앱버전 사용비율은 **공용 스킬로 분리**했습니다.
> - 릴리즈 기록의 앵커를 심사요청 스레드로 통일하고, 전체 출시 완료는 채널에도 알립니다.
>
> ### FE1-1370 — 와디태그(데이터 수집) 오류 수정 (7커밋)
> - 수집 오류를 여러 건 고쳤습니다 — **배너 슬롯이 상품 이벤트로 집계되던 문제**, 홈 검색바 본체 탭 수집 누락, 데이마케팅 검색바 태그의 지면 정보 오기, 스토어홈 PV URL 에 선택 카테고리 미반영, benefit PV.
> - iOS 의 짝은 FE1-1371 입니다.
>
> ### FE1-1392 — 웹 세션 핸드오프 (5커밋)
> - 앱의 OkHttp 세션을 웹뷰로 넘기는 경로를 정비했습니다. **세션 확립을 직렬화해 동시 실행을 차단**하고, 웹 세션을 갖고 있지 않은 경로의 주입 결함을 고쳤습니다.
> - **확립 실패 사유를 전수 계측**해 롤아웃 판독에 쓰도록 했고, 만료 세션을 재현하는 QA 스위치를 넣었습니다.
>
> ### FE1-1819 — 딥링크 상세 웹뷰의 LRU 상한 해체 (3커밋)
> - 딥링크로 연 상세 웹뷰의 **LRU 캐시 상한을 완전히 걷어냈습니다.** 보호 경로 판정을 `UrlKey` 실경로 기준으로 바꿨습니다.
> - 리뷰 반영으로 **해체 프로브에 DEBUG 가드**를 넣었습니다 — 릴리즈 빌드에서 메인 스레드 강제 GC 가 도는 것을 막기 위해서입니다.
>
> ### 기타
>
> | 이슈 | 내용 |
> |---|---|
> | FE1-1717 | 토스트 메시지를 **ComposeView 로 전환**하고 `BgDark` 타입 추가 |
> | FE1-1613 | 랩(실험실) 화면의 서버 선택 다이얼로그를 **Compose 로 마이그레이션** |
> | FE1-1577 | `network-config` 정리 |
> | FE1-1686 / FE1-1796 | 디버그 빌드 기본 설정을 **RC4 로 변경**, 디버그 빌드에서 A/B 테스트 설정 환경 제공 |
> | FE1-1770 | 검색홈 피드 재생 판정 디바운스 복원 (+ 죽은 코드 `resyncVisiblePlayback`·미사용 필드 제거) |
> | FE1-1804 | i18n 스킬 범위를 **시트 행 → TSV 산출까지로 축소** |
> | QA-23343 | 일본어 복귀 유도 모달(A타입) 문구 정렬 깨짐 수정 |
>
> ---

> 📅 **2026-09-03 main pull 보강** (120 커밋, v26.33.2 build 593 → **v26.36.0 build 594**)
>
> ℹ️ `main` 은 라이브 포인터라 릴리즈 시점에 한꺼번에 반영됩니다. 그래서 커밋 날짜는 2026-07-30 ~ 09-02 로 넓지만 **26.36.0 한 판의 작업 묶음**입니다. (버전 자동 업데이트·다국어 리소스 자동 동기화 커밋 다수 생략)
>
> **다국어 전면 적용(FE1-1549, 20커밋)** 과 **릴리즈 게시 자동화(FE1-1561, 13커밋)**, **따라잡기 글로벌 개방(FE1-1548, 9커밋)** 이 3대 테마입니다.
>
> ### FE1-1549 — 화면 문자열 다국어 전환 (20커밋)
> - GNB 탭·인증 카드·튜토리얼·알림 모달·완료 보너스 화면·카드 유형 배지·카드 오픈 날짜 배지 등 **화면 곳곳의 하드코딩 문자열을 다국어 키로** 옮겼습니다.
> - **헤더 파이프라인을 분리**하고, `dynamic-i18n` 캐시를 **언어별로 나눴습니다** — 언어를 바꿔도 이전 언어 캐시가 섞이지 않게 하려는 조치입니다.
> - 오픈 일시를 **기기 시간대로 환산**하도록 고쳤고(FE1-1549), 다국어 검증용 seam(테스트가 끼어들 지점)과 사문화된 리소스를 찾아내는 TC 스크립트를 추가했습니다. `Locale` 생성자 deprecated 경고도 해소했습니다.
>
> ### FE1-1561 / FE1-1569 — 릴리즈 게시 자동화 (15커밋)
> - Play 스토어 **게시 판정을 Gmail 확인이 아니라 `release status` API 로** 바꿨습니다. 메일 도착 시점에 의존하던 방식을 API 조회로 대체한 것입니다.
> - **롤아웃 게이트 기준을 크래시율에서 "신규 이슈 판별" 로 교체**했고, 출시 중단(halted) 상태의 릴리즈를 차단하는 가드와 `main`→`develop` 빈 머지 멱등 처리를 넣었습니다.
> - `scripts/release/` 신설 — `play-submit.sh`(116줄) · `play-release-status.sh` · `play-token.sh` · `play-track.sh` · `release-gate.sh`(115줄) · `release-config.json`, 그리고 **드라이런 하네스와 fixture 기반 테스트**(`test/dry-run.sh` 116줄, `test/run-tests.sh` 84줄, 트랙 상태 fixture 5종).
> - FE1-1569: 심사 감시 루틴이 **종료 상태를 먼저 확인**하도록 보강했습니다.
>
> ### FE1-1548 / FE1-1726 — 따라잡기·알림신청의 글로벌 개방 (12커밋)
> - 따라잡기 딥링크를 글로벌에 열고, `navigationMap` 의 `CATCHUP` 을 **글로벌 전용 신경로로 분리**했습니다. GNB 를 글로벌에서도 5탭으로 통일하고 뱃지 가드를 풀었으며, 글로벌에서는 알림 유도 모달을 띄우지 않게 막았습니다.
> - FE1-1726: **알림신청 URL 액션을 글로벌에서만 동작하도록 게이트**를 걸었습니다. 웹 FE1-1724/1771, iOS FE1-1722 와 짝입니다.
> - 검증용 TC 스크립트 `scripts/tc/catchup-global.sh`·`catchup-i18n.sh` 추가.
>
> ### 검색홈 개선 (FE1-1645 · FE1-1646 · FE1-1628, 9커밋)
> - **피드 영상 동시 재생**을 구현하고(6커밋), 검색홈 카드 타입 UI 와 CTR 상승을 위한 UI 를 개선했습니다. 피드 이미지에 리사이즈를 적용하고 **CDN 리사이즈 대상 호스트 게이트**를 고쳤습니다.
> - 성능 검증 TC `scripts/tc/video-feed-perf.sh`(214줄) 추가.
>
> ### 안정성 수정
>
> | 이슈 | 내용 |
> |---|---|
> | FE1-1526 | **세션 만료 → 로그인 이동 시 인트로 화면에 고착**되던 문제. 태스크를 초기화하도록 고치고, 초기화 완료를 기다리는 복원 로직이 반복 루프를 만드는 경로를 차단했습니다 (5커밋) |
> | FE1-1558 | 백그라운드 프로세스 기동 시 WebView 워밍업이 **data directory lock 크래시**를 유발하던 문제 |
> | FE1-1769 | 빈 JSON 노드를 리프(leaf)로 잘못 판정하던 문제 수정 |
> | QA-23090 | 데일리 미션 완료 툴팁을 눌러도 위시리스트 → 오픈예정 탭으로 이동하지 않던 현상 |
> | FE1-1728 | 앱 설정 fallback JSON 을 **clive 기준으로 최신화** |
>
> ### 기타
> - **FE1-1578** — 튜토리얼 액션 버튼을 앱에서 구현하고 액션 칩을 커스텀으로 전환, 로띠(Lottie) 영역 비율을 고정해 페이지 높이 흔들림을 없앴습니다.
> - **FE1-1690 / FE1-1720** — 따라잡기 화면 크기 대응(좌우 배치·가드·접근성 병합), 보너스 포인트 보드 하단 액션 버튼 최대 폭 560dp.
> - **FE1-1550** — 접근성 설정으로 애니메이션을 끈 기기에서의 로티 재생 대응.
> - **FE1-1498** — 테스트 하네스에 **공통 진입 헬퍼**(`enter_screen`·`ensure_logged_in`)와 하네스→앱 명령 프로토콜, 다이얼로그 자동 dismiss 를 넣었습니다. TC 규약 드리프트 센서도 함께.
> - **FE1-1747** — 이벤트 수집 누락건 4커밋 보강.
> - **FE1-1575** — 자체 서명 인증서 통과 판정을 한 곳으로 모으고, 앱에서 **로컬 웹 서버에 붙여 테스트**할 수 있게 했습니다.
>
> ---

> 📅 **2026-08-25 main pull 보강** (직전 갱신 2026-07-31 이후 345 커밋, v26.30.0 build 587 → **v26.33.2 build 593**)
>
> ℹ️ 직전 동기화 때는 로컬이 `cloud_live` 브랜치에 있었고 그 원격 브랜치가 없어 pull 이 실패했습니다. 이번에 **`main`(라이브 포인터)** 으로 옮겨 정상화했습니다.
>
> **URL 분기 글로벌 대응(FE1-1225)** 과 **RemoteConfig → AppSetting API 대체(FE1-1385)** 가 최대 테마이고, 홈 와디즈 에디션 카드·target SDK 36 상향·클라우드(rc4/CLIVE) 환경 구축이 뒤를 잇습니다. (버전 자동 업데이트 커밋 다수 생략)
>
> ### FE1-1225 — URL 분기 글로벌 대응 (56 커밋)
> - 국가·언어에 따라 진입 URL 을 가르는 분기를 글로벌까지 확장했습니다. 배선이 누락된 `UrlDirectoryHolder` 조회를 **Crashlytics 로 관측**하도록 해, 분기표에 없는 경로가 조용히 기본값으로 흐르지 않게 했습니다.
>
> ### FE1-1385 / FE1-1352 / FE1-1305 — RemoteConfig 를 AppSetting API 로 대체
> - Firebase RemoteConfig 의존을 걷어내고 **AppSetting API** 로 설정을 받도록 전환했습니다(FE1-1385, 27커밋). 이어서 A/B 테스트용 Remote Config 키를 제거하고 `featureControls` 의 미사용 필드를 정리했으며(FE1-1352), 미사용 Remote Config 키를 추가로 제거했습니다(FE1-1305). iOS 의 FE1-1297(RemoteConfig 이관)과 짝입니다.
>
> ### FE1-316 / SDK-16 — target SDK 36 상향
> - `targetSdk` 를 올리며 빌드 오류를 수정하고, 웹(Compose)·서비스홈 배너의 **적응형(edge-to-edge) 상태 신호**를 배선했습니다. TC 문서는 일반 웹뷰가 fragment(`ScreenRouteFeedWeb`) 경로를 타고 Compose 신호는 미커버라는 실측을 반영해 정정했습니다.
>
> ### FE1-1364 / FE1-1430 — 홈 환기영역 와디즈 에디션 카드 · 막펀잡기 레이아웃
> - 메인 홈 환기영역에 **와디즈 에디션 카드**를 추가했습니다(FE1-1364). 웹 FE1-1316/1318, iOS FE1-1316 과 같은 지면입니다.
> - FE1-1430: 막펀잡기 이미지 최대 크기를 커스텀 레이아웃으로 처리하고 tight-block 센터 정렬을 수정했으며, `i18n.json` 원복·시트 등록을 정리했습니다.
>
> ### FE1-1474 — 프로젝트 카드 `is_interested` 데이터 수집
> - 프로젝트 카드에 찜 여부(`is_interested`) 수집을 추가했습니다. 웹 FE1-1478, iOS FE1-1467 과 플랫폼 합의로 값 표기를 맞춘 작업입니다.
>
> ### FE1-1431 / FE1-1375 — 메인 홈 성능·전환 검증
> - FE1-1375: 메인홈 카드에 trace·experiment 를 주입하고, 리뷰를 거쳐 `StateFlow` 캡슐화·reified 콜사이트·encode 로그 구분을 정리했습니다.
> - FE1-1431: 전환 순서 검증 TC 스크립트를 추가하고, 실효가 없던 허수 리컴포지션 테스트는 제거해 **검증을 Perfetto·Layout Inspector 로 위임**했습니다(리빌 지연 0 문서 정합).
>
> ### FE1-1424 / FE1-1521 / FE1-1479 — 검색·SNB
> - FE1-1424: 검색홈 헤더의 인기 검색어가 늦게 도착할 때의 확장 애니메이션을 넣고, 최초 로드 Job 추적과 refresh 오류 시 `itemCount` 가드를 추가했습니다. 상태 신호·디바이스 검증 스크립트도 함께.
> - FE1-1521: SNB 리플·라운드·스켈레톤 폭 대응(아이콘 제외), 헤더 카테고리 수집 태그를 '카테고리'로, 검색홈 카테고리 클릭 action 을 '클릭'으로 정정.
> - FE1-1479: 검색 결과 노출 이벤트 수집 시 URL 에 쿼리를 포함하도록 수정.
>
> ### FE1-1412 / FE1-806 / FE1-1598 — 클라우드 환경 대응
> - FE1-1412: **rc4 환경 구축**. FE1-806: 앱 **CLIVE 테스트 지원** 및 유닛 테스트 실패 수정.
> - FE1-1598: `_waid` 쿠키 조회를 **서버 환경 기준으로** 변경하고, 승계 대상이 없는 경우도 완료로 기록하도록 했습니다(iOS FE1-1597 과 짝).
>
> ### 기타
> - FE1-1349: 스타트업(투자) 코드 제거 — 웹의 투자 지면 정리와 같은 방향입니다. FE1-1545: 미사용 GNB 웹뷰 인프라 제거 및 에디션 일반 웹페이지 TC 추가.
> - FE1-1326: 따라잡기 브레이즈 이벤트 추가. FE1-1350: `release-google-play.yml` 에서 Firebase 자동 테스트 실행 제거.
> - QA-22998: 메이커홈 로그아웃 시 오류 토스트가 노출되던 현상 수정. FE1-1525: 다이얼로그 dismiss 시 `IllegalStateException` 크래시 수정.
>
> ---
>
