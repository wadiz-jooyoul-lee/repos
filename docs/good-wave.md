...
> 📅 **2026-09-22 cloud_live pull 보강** (4 커밋)
>
> ### CLIENT-288 — 컨테이너 이미지 빌드 단계 정리
>
> 네 커밋 모두 `apps/good-wave/Dockerfile` 한 파일입니다.
>
> | 변경 | 내용 |
> |---|---|
> | 단계 이름 정리 | 빌드 단계를 `base`·`deps`·`builder`·`runner` 로 나눴습니다 |
> | 실행 단계 분리 | `runner` 를 **`node:22-bookworm-slim`** 으로 떼어냈습니다. 빌드 도구가 들어가지 않아 이미지가 가벼워집니다 |
> | 힙 상한 위치 | 빌드 힙 상한 선언을 `base` 에서 `builder` 로 옮겼습니다 |
> | 실행 사용자 | 실행 사용자 번호를 **차트의 `runAsUser` 와 같은 1000** 으로 맞췄습니다 |
>
> 마지막 항목이 눈에 띕니다.
> 컨테이너 안의 사용자 번호와 쿠버네티스가 지정하는 번호가 다르면 **파일 권한 문제가 생깁니다.**
