...
> 📅 **2026-09-17 main pull 보강** — `friendtalk-api` (12 커밋)
>
> 12커밋 전부 `DISPLAY-1726` 이고, **카탈로그형 브랜드 메시지 발송**이 주제입니다.
> 카탈로그형은 카카오톡 브랜드 메시지 중 **상품 카드를 여러 장 늘어놓는 형식**입니다.
>
> ### 카탈로그형 발송과 이미지 업로드
>
> - 카탈로그형(`CATALOG`) 브랜드 메시지 **발송과 이미지 업로드**를 지원합니다.
> - **비율별 이미지 업로드 API** 를 새로 만들었습니다. 카드 위치마다 필요한 이미지 비율이 다르기 때문입니다.
>   - 1:1 카탈로그 이미지는 **가운데를 잘라내는 방식(center-crop)** 으로 처리합니다.
>   - 첫 카드에 2:1 이미지를 따로 두려고 `oddFirstImageUrl` 필드를 넣었다가, 이틀 뒤 **되돌렸습니다.** 지금은 `imageUrl` 하나만 씁니다.
>   - 요청 필드를 복수형으로 바꿔 여러 비율을 한 번에 올립니다.
> - 발송 대행사별로 업로더가 나뉩니다. `MtsBrandMessageImageUploader`(+90줄)와 `NhnBrandMessageImageUploader`(+62줄)가 함께 손질됐습니다.
> - 업로드한 **원본 주소를 로그로 남깁니다.**
>
> ### 대체문자 제어
>
> 대체문자는 카카오톡 발송이 실패했을 때 문자로 보내는 대체 수단입니다.
>
> - **대체발송 지원 여부에 따라 대체문자 발송을 제어**합니다. 지원하지 않는 메시지는 문자로 넘기지 않습니다.
> - 카탈로그형의 대체문자 서브템플릿을 발송 쪽에 전달합니다.
> - 대체발송 변수 이름을 정리했습니다.
>
> ### 템플릿 등록 API 를 걷어냈습니다
>
> 이 서비스에 있던 **템플릿 등록 API 를 제거**했습니다.
> `BrandMessageTemplateService`(77줄)와 `TemplateValidator`(169줄), `TemplateV2Dto`(51줄)가 통째로 빠졌습니다.
> 템플릿 등록은 이제 [`platform-admin`](./platform-admin.md) 이 담당합니다. 이 서비스는 **발송만** 맡습니다.
>
> 짝이 되는 작업이 [`com.wadiz.api.funding`](./com.wadiz.api.funding/com.wadiz.api.funding.md) 에도 있습니다.
> `RWD-6026` 으로 친구톡 이미지 업로드를 v3 로 전환하고 카탈로그 규격 마커를 저장합니다.
