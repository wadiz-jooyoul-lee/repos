...
>
> 종전 실행기는 `scripts/run-tests.sh`(118줄) 였습니다. 이를 지우고 **`scripts/run-regression.mjs`(381줄)** 로 바꿨습니다.
> 세 가지가 함께 들어왔습니다.
>
> | 기능 | 내용 |
> |---|---|
> | `RUN_ID` 고정 | 한 번의 실행에 고유 번호를 붙여 유지합니다 |
> | 회차별 리포트 | 실행할 때마다 결과 리포트를 따로 남깁니다. 덮어쓰지 않습니다 |
> | 0건 감지 | **테스트가 한 건도 돌지 않은 경우를 잡아냅니다.** 설정이 어긋나 아무것도 실행되지 않았는데 성공처럼 보이는 일을 막습니다 |
>
> ### TC_006 국가 변경 시나리오 수정
>
> 국가를 바꾸는 시나리오에서 **기다리는 방식을 바꾸고 토스트 메시지 검증을 되살렸습니다**(`test/home/tc_006_country_change_save.spec.ts`).
> 앞선 회차에도 같은 시나리오의 시간 예산을 올린 적이 있어, 이 화면이 반복해서 불안정합니다.
